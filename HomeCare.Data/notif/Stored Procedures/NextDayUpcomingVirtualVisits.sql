CREATE PROCEDURE [notif].[NextDayUpcomingVirtualVisits]
...
BEGIN
	...
	DECLARE @VirtualVisitsBefore1Day [notif].[EventDataTable];

	INSERT INTO @VirtualVisitsBefore1Day
		SELECT 
			REF.[RefID],
			REF.[RefTableName],
		...
		CROSS APPLY (
			SELECT (	
				SELECT 
					...
					'Upcoming Virtual Visits - Next Day' [##EventName##]
				FOR JSON PATH
			) [Data]
		) TD
        WHERE
			...
			AND DATEADD(DAY, -1, SM.[StartDate]) BETWEEN @FromDateTime AND @ToDateTime;

	SELECT * FROM @VirtualVisitsBefore1Day;
