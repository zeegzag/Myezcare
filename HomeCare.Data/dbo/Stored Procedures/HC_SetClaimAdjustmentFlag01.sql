CREATE PROCEDURE [dbo].[HC_SetClaimAdjustmentFlag01]      
...
--PRINT @IsValidForUpdate        
 ...
 BEGIN        
  ...
  DECLARE db_cursor CURSOR FOR  SELECT DISTINCT BatchID, NoteID FROM BatchNotes BN WHERE BN.BatchID =@BatchID AND BN.NoteID = @NoteID      
  --WHERE BN.CLP01_ClaimSubmitterIdentifier=@PayorClaimNumber        
  OPEN db_cursor           
