CREATE PROCEDURE [dbo].[GetEmployeeReports]            
...
--SELECT * from ReportMaster RM where RM.IsDeleted=0 AND RM.Category='Employee' AND RM.IsDisplay=1            
select rm.ReportID,ReportName,SqlString,rm.ReportDescription,rm.DataSet,rm.RDL_FileName,rm.IsDeleted,rm.IsActive,rm.Category,rm.IsDisplay
...
where rpm.IsDeleted=0 and rm.IsDeleted=0 and rm.IsActive=1 and IsDisplay=1 AND RM.Category='Employee' and e.EmployeeID=@loggedInUser
