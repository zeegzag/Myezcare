...
select R.FirstName,R.LastName,dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat) AS ClientName,R.AHCCCSID,dbo.GetAge(r.Dob) as Age,R.Gender,                  
(CONVERT(VARCHAR(10),CONVERT(datetime,R.Dob ,1),101)) AS Dob,                    
 dbo.GetGenericNameFormat(CN.FirstName,'', CN.LastName,@NameFormat) as ParentName,CN.Phone1 as ParentPhone1,CN.Phone2 as ParentPhone2,                    
 ...
 (SELECT  STUFF((SELECT ',' + D.DXCodeName  FROM  DXCodes D inner join                       
   ...
   FOR XML PATH('')),1,1,'')) DXCodeName,(SELECT  STUFF((SELECT ',' + CT.FirstName +' ' +CT.LastName + ' '+ CT.Phone1                    
    ...
    dbo.GetGenericNameFormat(CM.FirstName,'', CM.LastName,@NameFormat) as  Facilitator ,sm.StartDate,sM.EndDate,r.PermissionForVoiceMail, R.NeedPrivateRoom,SM.Comments,SM.CancelReason  
