...
--END                  
 ...
 BEGIN            
  ...
  SELECT EmployeeTSDateID FROM EmployeeTimeSlotDates ETD WHERE ETD.EmployeeTimeSlotMasterID=@EmployeeTimeSlotMasterID AND ETD.EmployeeTSDate > @EndDate   AND ETD.EmployeeTSDate > @TodayDate          
  ...
  UPDATE ScheduleMasters
      SET IsDeleted = 1 WHERE EmployeeTSDateID IN (SELECT EmployeeTSDateID FROM @TempTable)            
