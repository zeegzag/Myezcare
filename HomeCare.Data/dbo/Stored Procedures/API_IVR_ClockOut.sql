...
BEGIN                  
 ...
 WHERE RPM.RoleID=@RoleID AND P.PermissionCode=@ApprovalRequiredIVRBypassPermission AND RPM.IsDeleted=0 AND @IsByPass = 1)    
 ...
 AND @ClockOutTime BETWEEN sm.StartDate and DateAdd(minute,30,sm.EndDate)
  --sm.EndDate BETWEEN @StartDateBefore AND @StartDateAfter OR @ClockOutTime >  
  AND sm.ReferralID IN (SELECT * FROM @tmpTable tt) AND ISNULL(SM.OnHold, 0) = 0 AND sm.IsDeleted=0          
    ...
    DECLARE @CurScheduleID bigint;  
       DECLARE eventCursor CURSOR FORWARD_ONLY FOR  
        ...
        WHILE @@FETCH_STATUS = 0 BEGIN  
            EXEC [dbo].[ScheduleEventBroadcast] 'EditSchedule-ClockOut', @CurScheduleID,'',''  
            FETCH NEXT FROM eventCursor INTO @CurScheduleID;  
        END;  
        CLOSE eventCursor;  
