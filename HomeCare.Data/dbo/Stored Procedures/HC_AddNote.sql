...
CREATE PROCEDURE [dbo].[HC_AddNote]                                                                    
...
BEGIN TRANSACTION trans                                                                          
 ...
 DailyUnitLimit INT,                                                        
  ...
  CONVERT(INT,(SumOfServiceTime/PerUnitQuantity + (CASE WHEN ((SumOfServiceTime % PerUnitQuantity)>= RoundUpUnit) THEN 1 ELSE 0 END )))          
   ...
   R.ReferralID,R.AHCCCSID,R.CISNumber,ServiceDate=CONVERT(DATE,EV.ClockInTime),EV.ClockInTime,EV.ClockOutTime,                                                        
   SC.ServiceCodeID,SC.ServiceCode,SC.ServiceName,SC.Description,                                                        
   PSC.MaxUnit,PSC.DailyUnitLimit,PSC.UnitType,PSC.PerUnitQuantity,ISNULL(SC.ServiceCodeType,0) AS ServiceCodeType,SC.ServiceCodeStartDate,SC.ServiceCodeEndDate,SC.IsBillable,                                                        
