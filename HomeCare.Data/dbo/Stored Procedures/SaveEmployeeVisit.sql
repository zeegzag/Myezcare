...
BEGIN              
 ...
 IF((SELECT DATEDIFF(MINUTE,(SELECT StartDate FROM ScheduleMasters WHERE ScheduleID=@ScheduleID),CONVERT(DATETIME, CONVERT(CHAR(8), (Select COALESCE(ClockInTime, [dbo].[GetOrgCurrentDateTime]()) From EmployeeVisits Where EmployeeVisitID=@EmployeeVisitID),
 112) + ' ' + CONVERT(CHAR(8), @ClockInTime, 108)))) < @BeforeClockIn)              
 BEGIN              
 SELECT -1 AS Result RETURN;              
 END        
 
   -- Check Billing Authorization
   DECLARE @tblReferralOutput Table (IsSuccess bit,ErrorCode VARCHAR(100),[Message] VARCHAR(250))
   INSERT INTO @tblReferralOutput(IsSuccess,ErrorCode,[Message])
   EXEC [dbo].[HC_ValidateReferralBillingAuthorization]  @ReferralID

  IF  EXISTS (
  SELECT * FROM @tblReferralOutput T          
    WHERE T.IsSuccess = 0
	)
	BEGIN
		SELECT -5         
...
DECLARE @PendingServiceTime int;      
 ...
 SET ClockInTime=@ClockInTime,              
    ClockOutTime=@ClockOutTime,UpdatedBy=@UpdatedBy, UpdatedDate=getdate()             
