...
BEGIN     
 ...
 WHEN @SortExpression = 'ClientName' THEN t.Name              
   ...
   select DISTINCT r.ReferralID,dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat) as Name,r.LastName,r.AHCCCSID,r.Gender, r.Dob ,dbo.GetAge(r.Dob) as Age ,rs.Status,e.EmployeeID AS Assignee,    
   dbo.GetGenericNameFormat(e.FirstName,e.MiddleName, e.LastName,@NameFormat) as AssigneeName,  
   p.ShortName as ContractName,dbo.GetGenericNameFormat(CM.FirstName,'', CM.LastName,@NameFormat) as FaciliatorName,  
   ...
   (SELECT STUFF((SELECT '|' + dbo.GetGenericNameFormat(e.FirstName,e.MiddleName, e.LastName,@NameFormat)  
   FROM Employees E           
