...
DECLARE @NameFormat VARCHAR(500) = dbo.GetOrgNameFormat()
 ...
 CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'Comment' THEN EDO.EmployeeComment END END ASC,                
   ...
   EDO.*, EmployeeName=dbo.GetGenericNameFormat(e.FirstName,e.MiddleName, e.LastName,@NameFormat), ActionTakenByName= dbo.GetGenericNameFormat(ATB.FirstName,ATB.MiddleName, ATB.LastName,@NameFormat),      
   SubmittedBy= dbo.GetGenericNameFormat(CB.FirstName,CB.MiddleName, CB.LastName,@NameFormat)      
   ...
   LEFT JOIN Employees ATB on ATB.EmployeeID=EDO.ActionTakenBy             --INNER JOIN Employees UB on UB.UpdatedBy=EDO.EmployeeID               
