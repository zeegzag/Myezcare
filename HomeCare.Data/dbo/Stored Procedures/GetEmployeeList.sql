...
--EXEC GetEmployeeList @IsDeleted = '0', @IsDepartmentSupervisor = '-1', @RecordAccess = '-1', @LoggedInUserID = '20173', @SortExpression = 'IsAbleToReceiveNotification', @SortType = 'DESC', @FromIndex = '1', @PageSize = '50'  
...
BEGIN    
  ...
  CASE    
          WHEN @SortType = 'ASC' THEN CASE    
              WHEN @SortExpression = 'IsAbleToReceiveNotification' THEN LEN(E.FcmTokenId)    
            END    
        END ASC,    
        CASE    
          WHEN @SortType = 'DESC' THEN CASE    
  ...
  CASE WHEN LEN(E.FcmTokenId) > 0 THEN 1 ELSE 0 END IsAbleToReceiveNotification  
  --ISNULL(e.[IsEnabled],0) as IsUserActive    
      FROM employees e    
