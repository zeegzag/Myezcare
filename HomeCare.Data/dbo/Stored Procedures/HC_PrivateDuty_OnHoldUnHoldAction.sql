...
BEGIN          
 ...
 BEGIN          
  ...
  WHERE SM.ReferralID=@ReferralID AND RTD.ReferralID=@ReferralID AND  RTD.ReferralTSDate BETWEEN @StartDate AND ISNULL(@EndDate,@InfinateDate) AND ISNULL(SM.OnHold, 0) = 0 AND SM.IsDeleted=0;      
  ...
  OUTPUT deleted.ScheduleID INTO @Output 
  FROM ScheduleMasters SM          
  INNER JOIN ReferralTimeSlotDates RTD ON RTD.ReferralTSDateID=SM.ReferralTSDateID          
