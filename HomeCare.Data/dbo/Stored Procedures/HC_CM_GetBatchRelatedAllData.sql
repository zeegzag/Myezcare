CREATE Procedure [dbo].[HC_CM_GetBatchRelatedAllData]                                                          
@BatchID as bigint ,                                                      
...
BEGIN                                                            
 ...
 GroupIDForMileServices=0,--CASE WHEN LEN(N.GroupIDForMileServices)=0 OR N.GroupIDForMileServices IS NULL THEN CONVERT(VARCHAR(100),NEWID()) ELSE N.GroupIDForMileServices END,                                
 ...
 BN.ClaimAdjustmentReason, BN.IsUseInBilling, N.StartTime,N.EndTime  , N.RenderingProvider_TaxonomyCode                                                 
 FROM Batches B                                                               
...
SELECT* FROM (                                            
 ...
 --SELECT BillingUnitLimit, PayorServiceCodeMappingID AS ID,NULL ReferralID,'' Type,POSStartDate StartDate,POSEndDate EndDate,IsDeleted,RevenueCode,'' AuthorizationCode                                 
 --FROM PayorServiceCodeMapping WHERE PayorServiceCodeMapping.PayorID=@PayorID                                
 -- UNION ALL           --SELECT BillingUnitLimit, ReferralBillingAuthorizationID AS ID,ReferralID,Type,StartDate,EndDate,IsDeleted,RevenueCode,AuthorizationCode FROM ReferralBillingAuthorizations WHERE ReferralBillingAuthorizations.PayorID=@PayorID     
