-- EXEC GetFacilityHouseList @FacilityName = '', @County = '', @RegionID = '0', @Phone = '', @NPI = '', @AHCCCSID = '', @EIN = '', @PayorApproved = '', @SortExpression = 'FacilityName', @SortType = 'DESC', @FromIndex = '1', @PageSize = '10'
CREATE PROCEDURE [dbo].[GetFacilityHouseList]
	...
	( 
		...
		(
			SELECT ROW_NUMBER() OVER (ORDER BY 
				...
				CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'Phone' THEN Phone END END ASC,
				CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'Phone' THEN Phone END END DESC,
