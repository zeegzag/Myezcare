CREATE PROCEDURE [dbo].[GetEdiFileLogList]                                
...
;WITH CTEEdiFileLog AS                            
 ...
 CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'FileSize' THEN  cast(EFL.FileSize as decimal)     END END DESC,            
   ...
   EFL.EdiFileLogID,EFL.EdiFileTypeID,EFL.FileName,EFL.FilePath,EFT.EdiFileTypeName,EMP.FirstName,EMP.LastName, StrDisplayName =dbo.GetGenericNameFormat(EMP.FirstName,EMP.MiddleName, EMP.LastName,@NameFormat),
   EFL.CreatedDate as AddedDate,EFL.BatchID,EFL.FileSize            
