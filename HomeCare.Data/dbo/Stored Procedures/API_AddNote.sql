...
--   INNER JOIN ScheduleMasters SM ON SM.ScheduleID=EV.ScheduleID -- and SM.isDeleted=0                                               
--   INNER JOIN Referrals R ON R.ReferralID = SM.ReferralID                                                              
--   INNER JOIN Employees E ON E.EmployeeID = SM.EmployeeID                         
--   LEFT JOIN ReferralTaskMappings RMT ON RMT.ReferralTaskMappingID=EVN.ReferralTaskMappingID -- and RMT.isDeleted=0                                                         
--   LEFT JOIN VisitTasks VT ON VT.VisitTaskID=RMT.VisitTaskID    and VT.isDeleted=0                                              
--   INNER JOIN ServiceCodes SC ON (SC.ServiceCodeID=VT.ServiceCodeID OR (SC.ServiceCodeID=EVN.ServiceCodeID AND EVN.ReferralTaskMappingID IS NULL)) AND SC.IsBillable=1-- and Sc.isDeleted=0            
...
--LEFT JOIN ReferralPayorMappings RPM ON RPM.ReferralID = R.ReferralID and RPM.isDeleted=0 LEFT JOIN DDMaster DM ON DM.DDMasterID = RPM.BeneficiaryTypeID AND DM.Title = @MedicaidType    
