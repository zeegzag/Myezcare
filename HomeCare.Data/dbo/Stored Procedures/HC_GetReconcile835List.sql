-- EXEC GetReconcile835List @ClaimNumber='89346ZRPB82BN40817', @Client='', @Get835ProcessedOnly='0', @Upload835FileID='0', @ServiceID = '-1', @ClaimAdjustmentTypeID = '-1', @SortExpression = 'ServiceDate', @SortType = 'DESC', @FromIndex = '1', @PageSize ='50'     
CREATE PROCEDURE [dbo].[HC_GetReconcile835List]                
@PayorID bigint =0,            
...
BEGIN                  
 ...
 SELECT *,COUNT(t1.NoteID) OVER() AS Count FROM               
  ...
  SELECT ROW_NUMBER() OVER (ORDER BY               
     ...
     --CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'BatchNoteID' THEN BatchNoteID END END ASC,              
     --CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'BatchNoteID' THEN BatchNoteID END END DESC ,              
  ...
  CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'LoadDate' THEN CONVERT(datetime, LoadDate, 103) END END DESC --,              
     --CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'LoadDate' THEN BatchNoteID END END ASC,              
     --CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'LoadDate' THEN BatchNoteID END END DESC               
     --CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'CreatedDate' THEN CONVERT(datetime, n.CreatedDate, 103) END END ASC,              
     --CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'CreatedDate' THEN CONVERT(datetime, n.CreatedDate, 103) END END DESC,              
   ...
   RowNumber FROM (     
    ...
    SELECT DISTINCT BN.MarkAsLatest , BN.S277, BN.S277CA,      
    BN.BatchNoteID,BN.BatchID, N.NoteID,     
    
 --Status= CASE WHEN CONVERT(DECIMAL(18,2),ISNULL(BN.CLP04_TotalClaimPaymentAmount,0))= 0 AND CSC.ClaimStatusName IS NOT NULL THEN 'Denied'      
 ...
 BN.LoadDate,BN.ReceivedDate,BN.ProcessedDate,      
    ...
    ROW_NUMBER() OVER ( PARTITION BY BN.NoteID,BN.BatchID Order BY BN.MarkAsLatest DESC, BN.BatchNoteID DESC) AS RowNumber       FROM BatchNotes BN             
