...
CREATE PROCEDURE [dbo].[Move_Notes_From_Tempporary_To_Permanent_CaseManagement]                
@PayorID BIGINT,                              
...
CalculatedUnit= 1,                      
N.NoteID,AddNewNote= CASE WHEN N.NoteID IS NULL THEN 1 ELSE 0 END,RTD.ReferralTSDateID ,RBA.ReferralBillingAuthorizationID , RenderingProvider_TaxonomyCode = DT.Value                              
...
FROM ReferralTimeSlotDetails RTDS                                
 ...
 INNER  JOIN Notes_Temporary NT ON NT.ReferralTimeSlotDetailID = RTDS.ReferralTimeSlotDetailID AND RTDS.UsedInScheduling = 1 AND NT.IsDeleted = 0  AND NT.ReferralTSDateID = RTD.ReferralTSDateID               
...
WHERE --SM.IsDeleted=0  AND SM.IsPatientAttendedSchedule=1                                 
 1=1   AND ( @ReferralsIds IS NULL OR LEN(@ReferralsIds)=0 OR NT.ReferralID in (SELECT val FROM GETCSVTABLE(@ReferralsIds))      )                        
...
------------------------------------------INSERT & UPDATE NOTES -------------------------------------------------------------------------------------------------                      
  ...
  ReferralTSDateID,CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,SystemID,ReferralBillingAuthorizationID,RenderingProvider_TaxonomyCode)                                                               
  ...
  @BillingProviderAddress,@BillingProviderCity,@BillingProviderState,@BillingProviderZipcode,                                  
  T1.ReferralTSDateID,GETDATE(),@LoggedInID,GETDATE(),@LoggedInID,@SystemID,T1.ReferralBillingAuthorizationID, T1.RenderingProvider_TaxonomyCode                               
