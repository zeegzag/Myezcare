...
begin                
 ...
 BEGIN                                    
   UPDATE EmployeeVisits SET SurveyCompleted=1,IsPCACompleted = 1,ActionTaken=2, UpdatedBy=CAST(@loggedInID as bigint) ,UpdatedDate=GETUTCDATE() WHERE EmployeeVisitID in (SELECT CAST(Val AS VARCHAR(100)) FROM GetCSVTable(@ListOfIdsInCsv));                                 
