...
BEGIN            
 ...
 WHERE sm.EmployeeID=@EmployeeID AND (sm.StartDate BETWEEN @StartDateBefore AND @StartDateAfter OR @ClockInTime BETWEEN sm.StartDate AND sm.EndDate) AND sm.ReferralID IN (SELECT * FROM @tmpTable tt) AND ISNULL(SM.OnHold, 0) = 0 AND sm.IsDeleted=0      
