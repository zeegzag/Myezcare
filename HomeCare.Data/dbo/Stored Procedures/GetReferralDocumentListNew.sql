...
INSERT INTO @TempTable(ReferralDocumentID,ComplianceID,Name,FilePath,KindOfDocument,CreatedBy,CreatedDate,UpdatedBy,UpdatedDate,IsDeleted,ExpirationDate,StoreType,GoogleFileId, IsOrbeonForm)              
SELECT ReferralDocumentID,RD.ComplianceID,Name=FileName,FilePath,KindOfDocument,CreatedBy=dbo.GetGenericNameFormat(E.FirstName,E.MiddleName,E.LastName,@NameFormat),RD.CreatedDate,                          
UpdatedBy=dbo.GetGenericNameFormat(EU.FirstName,EU.MiddleName,EU.LastName,@NameFormat),RD.UpdatedDate,RD.IsDeleted,ExpirationDate, StoreType,GoogleFileId, 0 as IsOrbeonForm              
...
WHERE UserID=ISNULL(@ReferralID,@EmployeeID) AND RD.UserType=@UserType   AND ((CAST(@IsDeleted AS bigint) = -1)    OR RD.IsDeleted = @IsDeleted)                        
...
--          FOR XML PATH('')), 1, 2, ''),                              
CreatedBy=dbo.GetGenericNameFormat(E.FirstName,E.MiddleName,E.LastName,@NameFormat),EBFM.CreatedDate,UpdatedBy=dbo.GetGenericNameFormat(EU.FirstName,EU.MiddleName,EU.LastName,@NameFormat),              
