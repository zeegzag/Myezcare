...
DECLARE @PTOTable Table(                  
 ...
 BEGIN                  
  ...
  LEFT JOIN ScheduleMasters SM ON ISNULL(SM.OnHold, 0) = 0 AND SM.IsDeleted=0 AND (SM.ReferralID=@ReferralID OR SM.EmployeeID=@NewEmployeeID) AND                   
 ...
 BEGIN              
  ...
  INNER JOIN @TempTable T ON SM.ReferralTSDateID=T.ReferralTSDateID AND SM.StartDate=T.StartDate AND SM.EndDate=T.EndDate AND ISNULL(SM.OnHold, 0) = 0 AND SM.IsDeleted=0              
 ...
 OR  (@StartDate BETWEEN RTD.ReferralTSStartTime AND RTD.ReferralTSEndTime  AND @EndDate BETWEEN RTD.ReferralTSStartTime AND RTD.ReferralTSEndTime)                
  ...
  LEFT JOIN ScheduleMasters SM ON  ISNULL(SM.OnHold, 0) = 0 AND SM.IsDeleted=0 AND SM.ScheduleID!=@ScheduleID AND (SM.ReferralID=@ReferralID OR SM.EmployeeID=@NewEmployeeID) AND                   
   (RTD.ReferralTSStartTime BETWEEN SM.StartDate AND SM.EndDate OR RTD.ReferralTSEndTime BETWEEN SM.StartDate AND SM.EndDate)                  
