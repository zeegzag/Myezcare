...
BEGIN                        
 ...
 BEGIN TRY      
  ...
  SELECT @ReferralTimeSlotMasterIds =       
   STUFF((SELECT ', ' + CONVERT(VARCHAR(50),ReferralTimeSlotMasterID)      
      ...
      WHERE ReferralID=@ReferralID AND ((StartDate>=@StartDate AND StartDate<=IsNull(@EndDate,@MaxDate)) OR  (EndDate>=@StartDate AND EndDate<=IsNull(@EndDate,@MaxDate)) OR   
		(@StartDate>=StartDate AND @StartDate<=IsNull(EndDate,@MaxDate)) OR (@EndDate>=StartDate AND @EndDate<=IsNull(EndDate,@MaxDate))) 
		AND (ReferralBillingAuthorizationID   IS NOT NULL AND ReferralBillingAuthorizationID > 0 )
     FOR XML PATH('')), 1, 2, '')      
  ...
  SELECT @ReferralTimeSlotMasterID,ReturnId, RESULT, 0        
  ...
  INSERT INTO       
   @ExistTimeSlotTable      
  SELECT       
   DISTINCT E.ReferralTimeSlotMasterID      
  FROM       
   ...
   AND E.IsDeleted=0 AND E.Day=T.Day AND ( E.ReferralBillingAuthorizationID   IS NOT NULL AND E.ReferralBillingAuthorizationID > 0 )     
  ...
  BEGIN        
   ...
   SELECT T.ReferralTimeSlotMasterID,T.Day,@StartTime,@EndTime,@UsedInScheduling,@Notes,@loggedInUserId,GETUTCDATE(),@loggedInUserId,GETUTCDATE(),@SystemID,@CareTypeId        
   FROM  @TempTable T         
   LEFT JOIN ReferralTimeSlotDetails RTSD ON RTSD.ReferralTimeSlotMasterID = T.ReferralTimeSlotMasterID AND RTSD.Day= T.Day AND RTSD.StartTime = @StartTime
   AND RTSD.EndTime = @EndTime AND RTSD.UsedInScheduling = @UsedInScheduling
   WHERE T.ExcludeFromInsert = 0   AND   RTSD.ReferralTimeSlotDetailID IS NULL   
 ...
 CROSS APPLY (    
  ...
  TSEndTimeNextDay = CONVERT(datetime, CONVERT(char(8), DATEADD(D, 1,T1.IndividualDate), 112) + ' ' + CONVERT(char(8), EndTime, 108))    
   ...
   LEFT JOIN (SELECT SDT.ReferralID, SDT.ReferralTSDateID, SDT.ReferralTSStartTime, SDT.ReferralTSEndTime, SD.CareTypeId FROM ReferralTimeSlotDates SDT   
   INNER JOIN ReferralTimeSlotDetails SD ON SDT.ReferralTimeSlotDetailID = SD.ReferralTimeSlotDetailID) ETSDT ON ETSDT.ReferralTSStartTime = T.ReferralTSStartTime   
