--  EXEC HC_DayCare_GetEmpRefSchOptions @ReferralID = '60053', @ScheduleID = '0', @SameDateWithTimeSlot = 'False', @StartDate = '2020/10/29 00:00:00', @EndDate = '2021/10/29 00:00:00'  
...
BEGIN                
 ...
 SELECT TOP 1 @ReferralTimeSlotMasterID=ReferralTimeSlotMasterID FROM ReferralTimeSlotMaster WHERE ReferralID=@ReferralID AND CONVERT(DATE,@StartDate) BETWEEN StartDate AND ISNULL (EndDate,@InfiniteEndDate) ORDER BY ReferralTimeSlotMasterID DESC   
...
ORDER BY DAY ASC,StartTime ASC                
    ...
    -- By Akhilesh Added Caretype   
 select distinct dm.DDMasterID as CareTypeID,dm.Title as CareTypeName   
 from ReferralTimeSlotDetails RTS  
 inner join DDMaster dm on dm.DDMasterID=RTS.CareTypeId  
 inner join ReferralTimeSlotMaster RTSM on RTSM.ReferralTimeSlotMasterID=RTS.ReferralTimeSlotMasterID  
 where RTSM.ReferralID=@ReferralID  
   
  
  --  select dds.DDMasterID as CareTypeID,dds.Title as CareTypeName from DDMaster dds        
