...
CREATE PROCEDURE [dbo].[GetEmployeesForNurseSchedule]    
...
BEGIN    
  ...
  -- AND ((CAST(@Address AS bigint) = 0)    
  --OR eg.val = CAST(@Address AS bigint))    
    
  AND ((@Address IS NULL OR LEN(@Address)=0) OR EXISTS (SELECT 1 FROM EmployeeGroup eg    
    WHERE eg.EmployeeID=e.EmployeeID AND eg.val in (SELECT CONVERT(BIGINT,VAL) FROM GetCSVTable(@Address))))      
