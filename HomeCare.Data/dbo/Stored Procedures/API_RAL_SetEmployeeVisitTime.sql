...
FROM dbo.RolePermissionMapping rpm INNER JOIN dbo.Permissions p ON rpm.PermissionID = p.PermissionID INNER JOIN dbo.Employees e ON e.RoleID = rpm.RoleID WHERE e.EmployeeID=@EmployeeID AND p.permissionid=(SELECT permissionid FROM permissions WHERE dbo.permissions.PermissionCode='Auto_Approved_Early_ClockIn') and rpm.isdeleted=0                
        
select  @ManualApprovedEarlyClockIn=PermissionCode              
FROM dbo.RolePermissionMapping rpm INNER JOIN dbo.Permissions p ON rpm.PermissionID = p.PermissionID INNER JOIN dbo.Employees e ON e.RoleID = rpm.RoleID WHERE e.EmployeeID=@EmployeeID AND p.permissionid=(SELECT permissionid FROM permissions WHERE dbo.permissions.PermissionCode='Manual_Approved_Early_ClockIn')  and rpm.isdeleted=0               
...
----------------------------------------Pallav Change End Here-----------------------  
  ...
  EXEC [dbo].[ScheduleEventBroadcast] @EventName, @ScheduleID,'',''                                                    
 SELECT 1;                                                                
 END                                                                
 ELSE                             
 BEGIN                                                                
