CREATE PROCEDURE [dbo].[GetReferralListForScheduling]        
 @RegioinID bigint=0,                      
...
DECLARE @NameFormat VARCHAR(500) = dbo.GetOrgNameFormat() 
 ...
 SELECT  *,COUNT(t1.ReferralID) OVER() AS Count FROM                       
  ...
  CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'NextAtt' THEN CONVERT(datetime, NextAttDate, 103)  END END DESC                                     
   ...
   , RegionCode, LastAttendedDate, Code, DefaultScheduleDays, UsedRespiteHours, ShortName, PlacementRequirement, ReferralSiblingMappingVlaue,NextAttDate  
  
   FROM  (  
   SELECT DISTINCT R.ReferralID,dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat) as Name,R.Gender,R.Dob, dbo.GetAge(R.Dob) Age, R.NeedPrivateRoom                      
   , P.PayorName, DrpTL.Location DropOffLocationName,DrpTL.TransportLocationID DropOffLocation,                      
   PickUPL.Location PickUpLocationName,PickUPL.TransportLocationID PickUpLocation,        --RequestDate=NULL,       
   dbo.GetNextScheduleRequestDate(ISNULL(R.ScheduleRequestDates,'')) as RequestDate,          
   Reg.RegionName,Reg.RegionCode,R.LastAttendedDate,F.Code,F.DefaultScheduleDays,rrp.UsedRespiteHours,P.ShortName,R.PlacementRequirement,            
   --(select min(StartDate) from ScheduleMasters where ReferralID=R.ReferralID and StartDate >= GETUTCDATE() and IsDeleted=0) as NextAttDate,       
    ...
    ROW_NUMBER() OVER ( PARTITION BY R.ReferralID ORDER BY SM.StartDate DESC) AS NextAttDateCount, NextAttDate=SM.StartDate,        
