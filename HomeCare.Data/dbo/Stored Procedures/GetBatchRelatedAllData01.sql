-- EXEC GetBatchRelatedAllData01 @BatchID = '99'  
CREATE Procedure [dbo].[GetBatchRelatedAllData01]  
@BatchID as bigint  
AS  
BEGIN  
...
BEGIN  
  ...
  ClaimSubmitterIdentifier=R.AHCCCSID,--CONVERT(VARCHAR,BN.NoteID)+ 'ZRPB'+CONVERT(VARCHAR,BN.BatchID) +'BN'+CONVERT(VARCHAR,BN.BatchNoteID),  
  PatientAccountNumber=R.AHCCCSID,--CONVERT(VARCHAR,BN.NoteID)+ 'ZRPB'+CONVERT(VARCHAR,BN.BatchID) +'BN'+CONVERT(VARCHAR,BN.BatchNoteID),  
  ...
  ,N.ServiceCodeEndDate,N.CheckRespiteHours,N.ModifierID,N.PosID,N.Rate,N.POSStartDate,N.POSEndDate,N.ZarephathService,N.StartMile,N.EndMile,N.StartTime,N.EndTime, CalculatedUnit=BN.CLM_UNIT  
  ...
  ,CalculatedAmount=BN.CLM_BilledAmount ,N.AttachmentURL,N.RandomGroupID,N.DriverID,N.VehicleNumber,N.VehicleType,N.PickUpAddress,N.DropOffAddress,N.RoundTrip,N.OneWay,N.MultiStops,N.EscortName  
  ,N.Relationship,N.DTRIsOnline,N.NoteComments,N.NoteAssignee,N.NoteAssignedBy,N.NoteAssignedDate,N.MonthlySummaryIds,PSM.BillingUnitLimit  
