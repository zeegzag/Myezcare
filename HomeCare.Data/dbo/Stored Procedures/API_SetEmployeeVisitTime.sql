...
Declare @IsAnytime bit
select @IsAnytime= AnyTimeClockIn from schedulemasters where scheduleid=@ScheduleID
if (@IsAnytime=1 and @ClockOutTime is null) -- Setting Starttime
begin 
 UPDATE ScheduleMasters            
   SET StartDate=CONVERT(DATETIME, CONVERT(CHAR(8), (Select COALESCE(StartDate,GETDATE()) From ScheduleMasters Where ScheduleID=@ScheduleID), 112) + ' ' + CONVERT(CHAR(8), @ClockInTime, 108)),          
    --EndDate=CONVERT(DATETIME, CONVERT(CHAR(8), (Select COALESCE(EndDate,GETDATE()) From ScheduleMasters Where ScheduleID=@ScheduleID), 112) + ' ' + CONVERT(CHAR(8), @ClockOutTime, 108)),          
 UpdatedDate=GETDATE(), UpdatedBy=@EmployeeID --,EmployeeID=@EmployeeID         
   WHERE ScheduleID = @ScheduleID;            
      EXEC [dbo].[ScheduleEventBroadcast] 'EditSchedule', @ScheduleID,'','' 
end
else if (@IsAnytime=1 and @ClockOutTime is not null) -- Setting EndTime
Begin
 UPDATE ScheduleMasters            
   SET 
    EndDate=CONVERT(DATETIME, CONVERT(CHAR(8), (Select COALESCE(EndDate,GETDATE()) From ScheduleMasters Where ScheduleID=@ScheduleID), 112) + ' ' + CONVERT(CHAR(8), @ClockOutTime, 108)),          
 UpdatedDate=GETDATE(), UpdatedBy=@EmployeeID ,VisitType=0--,EmployeeID=@EmployeeID         
   WHERE ScheduleID = @ScheduleID;            
      EXEC [dbo].[ScheduleEventBroadcast] 'EditSchedule', @ScheduleID,'','' 
End
  ...
  BEGIN    
      ...
      SET    
        ...
        IsApprovalRequired =    
                            CASE    
                              ...
                              WHEN @AutoApprovedEarlyClockIn IS NOT NULL AND @Type = 1 THEN 1    
    ...
    END    
    DECLARE @EventName NVARCHAR(MAX) = (SELECT 'EditSchedule' + CASE WHEN @Type=2 THEN '-ClockOut' ELSE '' END)
    EXEC [dbo].[ScheduleEventBroadcast] @EventName,    
                                        @ScheduleID,    
                                        '',    
                                        ''    
    SELECT    
