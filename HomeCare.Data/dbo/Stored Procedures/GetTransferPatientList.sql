--  EXEC GetTransferPatientList @StartDate = '2021/05/25', @EndDate = '2021/05/26', @SortExpression = 'Patient', @SortType = 'ASC', @FromIndex = '1', @PageSize = '10'              
CREATE PROCEDURE [dbo].[GetTransferPatientList]                    
@StartDate DATE =NULL,                                        
@EndDate  DATE=NULL,                     
...
;WITH CTEGetTransferPatientList AS                                      
 (                                       
...
convert(varchar,R.CreatedDate,101) as CreatedDate, EmpFirstName=E.FirstName, EmpLastName=E.LastName ,CreatedBy = dbo.GetGenericNameFormat(E.FirstName,E.MiddleName, E.LastName,@NameFormat)                  
FROM Referrals R               
INNER JOIN Employees E ON E.EmployeeID=R.CreatedBy                                
WHERE R.IsDeleted=0 AND R.ReferralStatusID=15 AND  
...
SELECT * FROM CTEGetTransferPatientList WHERE ROW BETWEEN ((@PageSize*(@FromIndex-1))+1) AND (@PageSize*@FromIndex)                                       
END  