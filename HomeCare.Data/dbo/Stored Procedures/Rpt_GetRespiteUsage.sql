...
SELECT  Agency, PayorName,ImportHours= ISNULL(ImportHours,0), AHCCCSID, CISNumber, dbo.GetGenericNameFormat(FirstName, MiddleName, LastName,@NameFormat) as Name            
...
Sep= ISNULL(SEP,0), StartDate=@StartDate, EndDate=@EndDate        
 FROM (              
 SELECT Agency,PayorName=ShortName,ImportHours, AHCCCSID,CISNumber ,FirstName,LastName,MiddleName, month,sum(Calulatedtime) as Calulatedtime FROM (              
   
 SELECT  Agency,ShortName,ImportHours, AHCCCSID, CISNumber ,FirstName,LastName,MiddleName,LEFT(DATENAME(MONTH,ISNULL(ServiceDate,@StartDate)),3)as [month],                          
  Calulatedtime= (CASE WHEN ServiceCode='S5151' THEN 24  * COUNT(ServiceCode)  
     ...
     ELSE 0 END)  
  FROM (  
      SELECT DISTINCT A.NickName As Agency,P.ShortName,R.ImportHours, R.AHCCCSID,R.CISNumber ,R.FirstName,R.LastName,R.MiddleName, N.ServiceDate,N.CalculatedUnit,  
      ...
      group by Agency,ShortName,ImportHours,AHCCCSID,CISNumber,FirstName,LastName,MiddleName, ServiceDate,ServiceCode              
) AS T              
 GROUP BY Agency,ShortName,CISNumber,ImportHours, AHCCCSID,FirstName,LastName,MiddleName, month              
   )                
 as s                
PIVOT                    
(                    
