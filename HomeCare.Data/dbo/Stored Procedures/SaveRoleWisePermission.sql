...
/*
Changed By Sagar
Commented all the direct saving statement to save by the save button
*/
...
--select @PermissionID=PermissionID from Permissions where permissioncode='Manual_Approved_Early_ClockIn'    
--IF(@ListOfPermissionIdInCsv like '%'+@PermissionID+'%')                
--  UPDATE RolePermissionMapping SET UpdatedBy=@EmployeeID, UpdatedDate=@CurrentDate, IsDeleted=1, SystemID=@SystemID                    
--  WHERE RoleID=@RoleID and PermissionID IN (@PermissionID)            
    
--select @PermissionID=PermissionID from Permissions where permissioncode='Auto_Approved_Early_ClockIn'    
--IF(@ListOfPermissionIdInCsv like '%'+@PermissionID+'%')                 
--  UPDATE RolePermissionMapping SET UpdatedBy=@EmployeeID, UpdatedDate=@CurrentDate, IsDeleted=1, SystemID=@SystemID                    
--  WHERE RoleID=@RoleID and PermissionID IN (@PermissionID)                 
