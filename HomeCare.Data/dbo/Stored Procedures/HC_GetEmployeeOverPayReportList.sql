-- EXEC [HC_GetEmployeeOverPayReportList] null,null,null,'EmployeeName','ASC','1','100',10027              
CREATE PROCEDURE [dbo].[HC_GetEmployeeOverPayReportList]      
...
SUM(DATEDIFF(minute,ev.ClockInTime,EV.ClockOutTime)) AS WorkingHour
FROM              
ScheduleMasters SM              
INNER JOIN EmployeeVisits EV ON EV.ScheduleID = SM.ScheduleID AND EV.IsDeleted=0 and IsPCACompleted=1--AND ((IsApprovalRequired=1 AND ActionTaken=2) OR (IsApprovalRequired=0) OR (IsApprovalRequired IS NULL))      
