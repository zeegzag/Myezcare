...
;WITH CTEReferralInternalMessage AS                      
 (                       
  ...
  (                      
   ...
   t.* FROM                      
   (        SELECT REF.ReferralID, REF.ClientID,REF.CISNumber,REF.AHCCCSID,REF.FirstName,REF.LastName, FullName = dbo.GetGenericNameFormat(REF.FirstName,REF.MiddleName, REF.LastName,@NameFormat), 
   Gender,  dbo.GetAge(REF.Dob) as Age ,                  
  RIM.ReferralInternalMessageID,RIM.Note,RIM.CreatedDate,RIM.IsResolved,    
   CreatedByName = dbo.GetGenericNameFormat(EMP1.FirstName,EMP1.MiddleName, EMP1.LastName,@NameFormat), 
  ResolvedByName = dbo.GetGenericNameFormat(EMP.FirstName,EMP.MiddleName, EMP.LastName,@NameFormat), RIM.ResolveDate ,          
  A.NickName AS AgencyName,Al.LocationName as AgencyLocationName, CaseManager = dbo.GetGenericNameFormat(C.FirstName,'', C.LastName,@NameFormat),  
  --P.PayorName,  
