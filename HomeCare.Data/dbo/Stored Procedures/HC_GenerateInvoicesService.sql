...
BEGIN
  ...
  CROSS APPLY (
    SELECT DATEDIFF(DAY, '1900-01-01', SM.StartDate) + 1 DayNum,
      DATEDIFF(WEEK, '1900-01-01', DATEADD(D, -1 * @WeekStartDay, SM.StartDate)) + 1 WeekNum,
      DATEDIFF(MONTH, '1900-01-01', SM.StartDate) + 1 MonthNum
    ) DT
  CROSS APPLY (
    SELECT DATEADD(D, @WeekStartDay - 1, DATEADD(WEEK, DT.WeekNum - 1, '1900-01-01')) WeekStart,
