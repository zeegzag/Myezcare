 ...
 SELECT  *,COUNT(t1.ReferralID) OVER() AS Count FROM                       
  ...
  WHEN @SortExpression = 'AssigneeName' THEN t.AssigneeName        
   WHEN @SortExpression = 'Address' THEN t.Address        
    ...
    t.* from (select DISTINCT r.ReferralID,dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat) as Name,r.ClientID,r.AHCCCSID,r.ReferralStatusID,r.IsSaveAsDraft,r.CISNumber,r.Gender, r.Dob, dbo.GetAge(r.Dob) as Age ,                     
   RS.Status,r.Assignee,dbo.GetGenericNameFormat(e.FirstName,e.MiddleName, e.LastName,@NameFormat) as AssigneeName,--rp.PayorID,p.ShortName as ContractName,  
   r.PlacementRequirement,                      
   r.CaseManagerID,dbo.GetGenericNameFormat(cm.FirstName,'', cm.LastName,@NameFormat) as FaciliatorName,r.ZSPLifeSkills,r.ZSPRespite,r.ZSPCounselling,r.CreatedDate,r.CreatedBy, dbo.GetGenericNameFormat(eself.FirstName,eself.MiddleName, eself.LastName,@NameFormat) as CreatedName,                      
   a.NickName as CompanyName,al.LocationName,rc.IsCheckListCompleted,rc.ChecklistCompletedBy,dbo.GetGenericNameFormat(er.FirstName,er.MiddleName, er.LastName,@NameFormat) as CheckListName,rc.ChecklistCompletedDate,                      
   rsf.IsSparFormCompleted,rsf.SparFormCompletedBy,dbo.GetGenericNameFormat(es.FirstName,es.MiddleName, es.LastName,@NameFormat) as ClinicalReviewName,rsf.SparFormCompletedDate,r.NotifyCaseManager,                      
   r.AgencyID,r.AgencyLocationID,r.UpdatedDate,dbo.GetGenericNameFormat(eselfUP.FirstName,eselfUP.MiddleName, eselfUP.LastName,@NameFormat) as UpdatedName, r.IsDeleted,r.CounselingService,r.LifeSkillsService,r.RespiteService,r.ClientNickName,c.Address,c.City,c.ZipCode,c.State        
   from Referrals r                      
