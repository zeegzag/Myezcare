...
CREATE PROCEDURE [dbo].[API_AddNoteByCareType_01]                                            
...
--		WHERE RBA.IsDeleted = 0 AND SM.IsDeleted = 0 AND EV.IsDeleted = 0 and sm.ScheduleID=@ScheduleID
  ...
  INSERT INTO @Temp    (RankOrder ,ReferralID ,AHCCCSID ,CISNumber ,ServiceDate ,ClockInTime ,ClockOutTime , ServiceCodeID ,ServiceCode , ServiceName ,Description ,  MaxUnit ,DailyUnitLimit ,UnitType ,PerUnitQuantity ,ServiceCodeType ,ServiceCodeStartDate ,
  ServiceCodeEndDate ,IsBillable ,EmployeeID ,  EmployeeFirstName ,EmployeeLastName ,HHA_NPI_ID ,Address ,City ,StateCode ,ZipCode ,PayorID ,PayorName , PayorShortName ,PayorAddress ,PayorIdentificationNumber ,PayorCity ,PayorState ,PayorZipCode ,EmployeeVisitNoteID ,
NoteID ,POSStartDate ,POSEndDate ,Rate ,SumOfServiceTime , ModifierID , AddNewNote ,RoundUpUnit ,ReferralBillingAuthorizationID,SumOfUnit)                                                            
  SELECT *,                  
...
R.ReferralID, RPM.BeneficiaryNumber as 'AHCCCSID', --R.AHCCCSID,      
   ...
   RBA.RoundUpUnit,RBA.ReferralBillingAuthorizationID      
FROM            
  ...
  --      ServiceCodes AS SC ON VT.ServiceCodeID = SC.ServiceCodeID left JOIN  -- Changed by Pallav as the notes were not getting added due to missing Auth Numbers 
		...
		Notes N ON N.ServiceCodeID=SC.ServiceCodeID AND N.ReferralID=R.ReferralID  AND  N.EmployeeVisitID=EV.EmployeeVisitID and (N.IsDeleted=0 OR N.IsDeleted IS NULL)   LEFT JOIN    
  DDMaster DM ON DM.DDMasterID = RPM.BeneficiaryTypeID AND DM.Title = @MedicaidType   
WHERE EV.EmployeeVisitID =@EMPLOYEEVISITID  and ev.isDeleted=0 and ev.isPCACompleted=1 --AND (N.IsDeleted=0 OR N.IsDeleted IS NULL)    
  ...
  CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,SystemID,ReferralBillingAuthorizationID)                                                              
  ...
  --        FOR XML PATH (''))                                                              
  --        , 1, 1, ''),                                                              
GETDATE(),@EmployeeID,GETDATE(),@EmployeeID,@SystemID,ReferralBillingAuthorizationID                                                   
