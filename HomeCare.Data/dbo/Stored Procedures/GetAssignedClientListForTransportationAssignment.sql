...
DECLARE @NameFormat VARCHAR(500) = dbo.GetOrgNameFormat() 
 select TG.*, (SELECT           
     ...
     (SELECT           
     STUFF((SELECT '/ ' + convert(varchar(max),dbo.GetGenericNameFormat(e.FirstName,e.MiddleName, e.LastName,@NameFormat))          
     FROM TransportationGroupStaffs s           
   ...
   dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat) Name,R.Gender,dbo.GetAge(R.Dob) Age, ss.ScheduleStatusID,ss.ScheduleStatusName  ,         
   dbo.GetGenericNameFormat(C.FirstName,'', C.LastName,@NameFormat) as ParentName, C.Phone1 Phone,R.IsDeleted AS IsReferralDeleted          
