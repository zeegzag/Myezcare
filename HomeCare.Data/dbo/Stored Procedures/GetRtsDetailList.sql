...
CREATE PROCEDURE [dbo].[GetRtsDetailList]                
 ...
 @CareTypeID BIGINT = 0   
 as
--select @ReferralTimeSlotMasterID = '60504', @CareTypeID = '41', @SortExpression = 'Day', @SortType = 'ASC', @FromIndex = '1', @PageSize = '50'                            
BEGIN                                
 ...
 CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'Title' THEN d.Title END END ASC,                            
   ...
   etd.IsDeleted,etd.StartTime,etd.EndTime,etd.Day,etd.Notes,etd.UsedInScheduling,ets.CareTypeID,d.Title,etd.AnyTimeClockIn, etd.ReferralBillingAuthorizationID,RBA.AuthorizationCode            
   ...
   LEFT JOIN DDMaster d     (NOLOCK)     ON d.DDMasterID=ets.CareTypeId      
   LEFT JOIN ReferralBillingAuthorizations RBA  ON RBA.ReferralBillingAuthorizationID=ETD.ReferralBillingAuthorizationID    
