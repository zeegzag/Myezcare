--   EXEC GetCaptureCallList @IsDeleted = '0', @SortExpression = 'Id', @SortType = 'DESC', @FromIndex = '1', @PageSize = '50'            
 --   EXEC GetCaptureCallList @Name = 'sushil', @Email = 'ravi321@gmail.com', @Address = 'Seymour', @IsDeleted = '0', @SortExpression = 'Id', @SortType = 'DESC', @FromIndex = '1', @PageSize = '50'                     
...
BEGIN                  
   ;WITH CTECaptureCallList AS(                     
     ...
     END DESC                    
       ...
       * FROM (                  
       ...
       FROM CaptureCall CC            
    LEFT JOIN Employees E ON E.EmployeeID=cast(CC.EmployeesIDs as bigint)   --CC.CreatedBy       
 ...
 AND (CC.IsDeleted=0 or CC.IsDeleted is null )        
	...
	AND ((@Status IS NULL OR LEN(@Status)=0) OR CC.Status LIKE '%' + @Status + '%')
	AND ((@Notes IS NULL OR LEN(@Notes)=0) OR CC.Notes LIKE '%' + @Notes + '%')  	                
      ) AS T2) AS T1)                  
