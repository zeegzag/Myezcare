...
BEGIN                                        
 ...
 CASE WHEN @SortType = 'ASC' THEN                                      
   ...
   VisitTaskDetail = ISNULL(v.VisitTaskDetail,ev.Description),ev.Description,ev.ServiceTime,ev.IsDeleted, VTF.TaskForms             
   ...
   OUTER APPLY (SELECT COUNT(1) TaskForms FROM TaskFormMappings TFM WHERE TFM.VisitTaskID = v.VisitTaskID AND TFM.IsDeleted = 0) VTF         
 ...
 AND                     
   ((@PatientName IS NULL OR LEN(@Name)=0)                       
   OR (                      
