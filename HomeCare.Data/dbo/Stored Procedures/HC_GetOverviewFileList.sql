...
PRINT 'Non SENT'+ @NonSENTBatches            
  ...
  SELECT             
  R.ReferralID,BN.NoteID,BN.BatchID,ClientName=dbo.GetGenericNameFormat(r.FirstName,r.MiddleName, r.LastName,@NameFormat),R.FirstName,R.LastName,        
...
--FOR XML PATH('')),1,1,'')) AS ContinuedDX,            
  (CASE WHEN N.ServiceDate >'2017-10-01' AND RBA.BillingUnitLimit IS NOT NULL AND N.CalculatedUnit> RBA.BillingUnitLimit THEN CONVERT(DECIMAL(10,2),             
  (N.CalculatedAmount / N.CalculatedUnit) * RBA.BillingUnitLimit ) ELSE CONVERT(DECIMAL(10,2),N.CalculatedAmount) END ) as CalculatedAmount,        
  ...
  INNER JOIN  ReferralBillingAuthorizations RBA on RBA.ReferralBillingAuthorizationID= N.ReferralBillingAuthorizationID  
    
    
  --RBA.ReferralID=N.ReferralID and RBA.ServiceCodeID=N.ServiceCodeID and Rba.PayorID=N.PayorID and     
  --RBA.ReferralBillingAuthorizationID=(select Top 1 ReferralBillingAuthorizationID from ReferralBillingAuthorizations rba1     
  --         where n.servicedate between rba1.StartDate and rba1.endDate and rba1.IsDeleted=0 and rba1.ReferralID=n.ReferralID     
  --         and n.PayorID=rba1.PayorID and n.ServiceCodeID=rba1.ServiceCodeID)    
  ...
  INNER JOIN DDMaster DM on DM.DDMasterID = RBA.CareType        
...
--FOR XML PATH('')),1,1,'')) AS ContinuedDX,            
 ...
 --ModifierName = M.ModifierCode            
  ...
  --INNER JOIN  ReferralBillingAuthorizations RBA on RBA.ReferralID=N.ReferralID and RBA.ServiceCodeID=N.ServiceCodeID and Rba.PayorID=N.PayorID and     
  --RBA.ReferralBillingAuthorizationID=(select Top 1 ReferralBillingAuthorizationID from ReferralBillingAuthorizations rba1     
  --         where n.servicedate between rba1.StartDate and rba1.endDate and rba1.IsDeleted=0 and rba1.ReferralID=n.ReferralID     
  --         and n.PayorID=rba1.PayorID and n.ServiceCodeID=rba1.ServiceCodeID)    
