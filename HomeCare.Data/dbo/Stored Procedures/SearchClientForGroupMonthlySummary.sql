...
CREATE PROCEDURE [dbo].[SearchClientForGroupMonthlySummary]        
...
select distinct top (@PageSize) R.ReferralID,R.LastName+', '+ R.FirstName as Name,R.AHCCCSID,R.CISNumber,C.Phone1,C.Address,RPM.PayorID,P.ShortName as PayorName,  CMG.FirstName+', '+ CMG.LastName AS CaseManager,      
...
'|'+ISNULL(CONVERT(CHAR(20), RD.EndDate,101),'')+'|'+CONVERT(CHAR(20), RD.Precedence)+'|'+D.DxCodeType+'|'+DT.DxCodeShortName+'|'+D.DXCodeWithoutDot        
  ...
  WHERE RD.ReferralID = R.ReferralID and RD.IsDeleted=0            
 ...
 from Referrals R        
 INNER JOIN CaseManagers CMG ON CMG.CaseManagerID=R.CaseManagerID
