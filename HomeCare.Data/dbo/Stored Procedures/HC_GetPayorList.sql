-- exec [HC_GetPayorList] '','','','',-1,-1,'PayorID','ASC',1,50           
CREATE PROCEDURE [dbo].[HC_GetPayorList]                
...
;WITH CTEPayor AS                      
 ...
 CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'PayorBillingType' THEN PayorBillingType END END ASC,                      
 CASE WHEN @SortType = 'DESC' THEN CASE WHEN @SortExpression = 'PayorBillingType' THEN PayorBillingType END END DESC,                
 ...
 PayorID,PayorName,ShortName,AgencyNPID,DM.Title as PayerGroup,P.PayorBillingType,PayorIdentificationNumber,NPINumber,            
 CONCAT(                   
 ...
 P.IsDeleted                            
 FROM  Payors P                
 LEFT JOIN DDmaster DM on DM.DDMasterID =P.PayerGroup               
 --LEFT JOIN PayorTypes PT on PT.PayorTypeID=P.PayorTypeID                
   WHERE ((@IsDeleted = -1) OR P.IsDeleted=@IsDeleted)               
   ...
   AND ((@PayorBillingType IS NULL OR LEN(@PayorBillingType)=0) OR (PayorBillingType LIKE '%' + @PayorBillingType+ '%'))          
   AND           
   (          
