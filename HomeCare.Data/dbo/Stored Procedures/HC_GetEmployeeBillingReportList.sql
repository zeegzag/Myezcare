...
BEGIN      
  ...
  (      
    SELECT      
      ...
      ,STRING_AGG(OT.VisitIDs, ',')VisitIDs    
  FROM      
    (      
      SELECT      
        T.EmployeeID,      
        ...
        STRING_AGG(ISNULL(T.VisitIDs, 0), ',') VisitIDs,    
  CASE      
      ...
      FROM dbo.EmpHours((SELECT STRING_AGG(EmployeeID, ',') FROM FilteredEmployees), @StartDate, @EndDate, @WeekStartDay) T      
  ...
  CASE      
      ...
      (      
        SELECT      
          ...
          T.AllocatedHour,      
          T.PTOHour,      
          cast((SELECT CONVERT(varchar(2), SUM(DATEDIFF(minute, ClockInTime, ClockOutTime)) / 60) + '.' + RIGHT('0' + CONVERT(varchar(2), SUM(DATEDIFF(minute, ClockInTime, ClockOutTime)) % 60), 2) as TotalHours    
   FROM EmployeeVisits  WHERE EmployeeVisitID IN (select val from  GetCSVTable(T.VisitIDs))) AS numeric(36, 2))WorkingHour,    
          ...
          WorkingHourInStr =   cast( (SELECT CONVERT(varchar(2), SUM(DATEDIFF(minute, ClockInTime, ClockOutTime)) / 60) + '.' + RIGHT('0' + CONVERT(varchar(2), SUM(DATEDIFF(minute, ClockInTime, ClockOutTime)) % 60), 2) as TotalHours    
  FROM EmployeeVisits  WHERE EmployeeVisitID IN (select val from  GetCSVTable(T.VisitIDs))) as varchar(9)),    
