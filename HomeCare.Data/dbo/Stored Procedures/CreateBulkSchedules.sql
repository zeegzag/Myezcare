...
BEGIN          
 ...
 AND SM.CareTypeId = @CareTypeId      
 AND ISNULL(SM.AnyTimeClockIn, 0) = 0      
 AND (@PayorID = 0 OR ISNULL(SM.PayorID, 0) = 0 OR SM.PayorID = @PayorID)      
    AND SM.IsDeleted = 0          
...
-- Check Billing Authorization
  ...
  AND RTD.ReferralTSDate = ETD.EmployeeTSDate  
        AND (          
        (@SameDateWithTimeSlot = 0          
    ...
    AND RTD.ReferralTSEndTime BETWEEN @StartDate AND @EndDate)          
      ...
      LEFT JOIN (      
  SELECT SM.* FROM ScheduleMasters SM          
  INNER JOIN ReferralTimeSlotDates RTD          
        ON RTD.ReferralID = @ReferralID          
   AND RTD.ReferralTSDateID = SM.ReferralTSDateID      
  ...
  AND CONVERT(DATE, SM.startdate) BETWEEN @startdate AND @enddate         
        AND SM.CareTypeId = @CareTypeId      
  AND (LEN(@ReferralTimeSlotDetailIDs) = 0 OR RTD.ReferralTimeSlotDetailID IN          
     (          
       SELECT          
      CONVERT(bigint, VAL)          
   ...
   ON (SM.EmployeeTSDateID = ETD.EmployeeTSDateID)      
