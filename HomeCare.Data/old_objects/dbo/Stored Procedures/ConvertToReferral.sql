...
-----------------------------------------------------------------------------------------------                            
    ...
    (FirstName,LastName,CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,SystemID,ReferralTrackingID,Gender,ClientID)                                    
    VALUES                                                      
    (@FirstName,@LastName,getdate(),1,getdate(),1,0,@Id,'M',@ClientID);       
 ...
 INSERT INTO ContactMappings(ContactID,ReferralID,ClientID,IsEmergencyContact,ContactTypeID,Relation,CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,SystemID)      
  VALUES(@ContactID,@ReferralID,@ClientID,0,1,'',getdate(),1,getdate(),1,0)      
 ---------------------------------------------------------------------------------------------------------------    
 IF(@ReferralID>0)  
 BEGIN
 ...
 END 
--declare  @ReferralID bigint                                     
    SELECT top 1  ReferralID from referrals where ReferralTrackingID= @Id   return;                          
END 