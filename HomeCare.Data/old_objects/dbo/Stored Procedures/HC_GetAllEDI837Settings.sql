...
select @colsUnpivot = stuff((select ','+  C.name +'= CASE WHEN '+C.name+' IS NULL THEN '''' ELSE CAST('+C.name+' as NVARCHAR(MAX)) END' --quotename(C.name)                
  ...
  AND C.name!='RequiredRenderingProvider' AND C.name!='SkipSituationalContent' AND C.name!='CheckForPolicyNumber'     
  ...
  AND C.name!='I_PVR03_ReferenceIdentification' AND C.name!='Claim_CLM010_PatientSignatureSource'     
