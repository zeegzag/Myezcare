CREATE PROCEDURE [dbo].[AddRtsMaster] @ReferralTimeSlotMasterID BIGINT,
  @ReferralID BIGINT,
  @StartDate DATE,
  @EndDate DATE = NULL,
...
BEGIN
  ...
  BEGIN TRY
	...
	--             AND E.ReferralID in (@ReferralID) AND E.ReferralTimeSlotMasterID!=@ReferralTimeSlotMasterID AND E.IsDeleted=0)          
    -- BEGIN                  
    ...
    BEGIN
      ...
      FROM (
        SELECT E.ReferralID,
          ...
          ReferralTSStartTime = CONVERT(DATETIME, CONVERT(CHAR(8), T1.IndividualDate
              , 112) + ' ' + CONVERT(CHAR(8), StartTime, 108)),
          ReferralTSEndTime = CONVERT(DATETIME, CONVERT(CHAR(8), T1.IndividualDate, 
              112) + ' ' + CONVERT(CHAR(8), EndTime, 108)),
