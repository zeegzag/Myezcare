...
R.ReferralID, RPM.BeneficiaryNumber as 'AHCCCSID', --R.AHCCCSID,      
   ...
   RBA.MaxUnit,RBA.DailyUnitLimit,RBA.UnitType,RBA.PerUnitQuantity,ISNULL(SC.ServiceCodeType,0) AS ServiceCodeType,        
...
FROM            
  ...
  Employees AS E ON SM.EMPLOYEEID=E.EMPLOYEEID INNER JOIN  
        ...
        ReferralBillingAuthorizations AS RBA  ON RBA.ReferralID = RPM.ReferralID AND RBA.ReferralBillingAuthorizationID = SM.ReferralBillingAuthorizationID INNER JOIN 
		Payors AS P ON RPM.PayorID = P.PayorID  INNER JOIN  
		...
		Notes N ON N.ServiceCodeID=VT.ServiceCodeID AND N.ReferralID=R.ReferralID  AND  N.EmployeeVisitID=EV.EmployeeVisitID and (N.IsDeleted=0 OR N.IsDeleted IS NULL)   LEFT JOIN   
  DDMaster DM ON DM.DDMasterID = RPM.BeneficiaryTypeID AND DM.Title = @MedicaidType   
WHERE EV.EmployeeVisitID =@EMPLOYEEVISITID AND VT.VisitTaskType='Task' and ev.isDeleted=0 and ev.isPCACompleted=1 --AND (N.IsDeleted=0 OR N.IsDeleted IS NULL)    
) AS T                                                              
