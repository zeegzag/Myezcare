...
CREATE PROCEDURE [dbo].[API_GetPatientDetail] @ReferralID BIGINT,
...
BEGIN
  ...
  SELECT r.ReferralID,
    ...
    c.Email,
    Phone = c.Phone1,
    EmergencyPhone = ec.Phone1,
    sm.ScheduleID,
    sm.CareTypeTimeSlotID
  FROM dbo.Referrals r
  OUTER APPLY (
    SELECT MAX(SM.ScheduleID) ScheduleID
    FROM ScheduleMasters sm
    LEFT JOIN [dbo].[EmployeeVisits] ev
    ...
    WHERE sm.ReferralID = r.ReferralID
      ...
      AND Sm.IsDeleted = 0
      AND (
        ISNULL(ev.IsPCACompleted, 0) <> 1
        OR ev.ClockOutTime IS NULL
        OR sm.EndDate < @ServerCurrentDate
        )
    GROUP BY CONVERT(DATE, sm.StartDate)
    ) MS
  LEFT JOIN ScheduleMasters sm
    ON MS.ScheduleID = SM.ScheduleID
  LEFT JOIN DDMaster l
    ON l.DDMasterID = r.LanguageID
  LEFT JOIN dbo.ContactMappings cm
    ON r.ReferralID = cm.ReferralID
      AND cm.ContactTypeID = 1
  LEFT JOIN dbo.ContactMappings ecm
    ON r.ReferralID = ecm.ReferralID
      AND ecm.IsEmergencyContact = 1
  LEFT JOIN dbo.Contacts c
    ON cm.ContactID = c.ContactID
      AND c.IsDeleted = 0
  LEFT JOIN dbo.Contacts ec
    ON ecm.ContactID = ec.ContactID
      AND ec.IsDeleted = 0
  WHERE r.IsDeleted = 0
