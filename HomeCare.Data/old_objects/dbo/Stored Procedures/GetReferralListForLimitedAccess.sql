CREATE PROCEDURE [dbo].[GetReferralListForLimitedAccess]                      
 @AssigneeID bigint= 0,                    
...
BEGIN                        
 ...
 SELECT * FROM CTEReferralList WHERE ROW BETWEEN ((@PageSize*(@FromIndex-1))+1) AND (@PageSize*@FromIndex)  AND ReferralID =  (SELECT DISTINCT ReferralID FROM ScheduleMasters WHERE EmployeeID = @employeeId)  --AND Assignee = @AssigneeID         
