...
BEGIN              
 ...
 Begin   
  ...
  else if(@BulkType ='ServiceCode')        
   UPDATE VisitTasks set  ServiceCodeId=@VisitTaskValue ,UpdatedDate=GETUTCDATE(),UpdatedBy=@loggedInID where VisitTaskID in (select val from dbo.f_split(@VisitTaskIDList, ','))    
 ...
 EXEC UpdateReferralTaskMappings @VisitTaskIDList, @loggedInID, '::1' --TODO
