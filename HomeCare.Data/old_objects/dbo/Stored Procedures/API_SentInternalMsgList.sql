...
SELECT @UnreadMsgCount=COUNT(ReferralInternalMessageID) FROM ReferralInternalMessages WHERE Assignee=@EmployeeID AND IsDeleted=0 AND IsResolved=0         
 ...
 (                           
  ...
  SELECT ReferralInternalMessageID,Message=Note,IsResolved,ResolveDate,ResolvedComment, PatientName= dbo.GetGeneralNameFormat(R.FirstName,R.LastName), AssignedBy=EC.FirstName+' '+EC.LastName, AssignedDate=RIM.CreatedDate, UnreadMsgCount=@UnreadMsgCount   
