 ...
 BEGIN TRY  
  IF (@EmployeeID IS NOT NULL)  
   BEGIN  
    ...
    ELSE IF (@Action = 2)  
     BEGIN  
      UPDATE Employees SET Password=@Password,PasswordSalt=@PasswordSalt WHERE MobileNumber=@MobileNumber  
     END  
    ELSE IF (@Action = 3)  
     BEGIN  
   ...
   IF @@TRANCOUNT > 0  
    ...
    END  
   SELECT FirstName,LastName,MobileNumber,Password FROM dbo.Employees e WHERE MobileNumber=@MobileNumber;  
