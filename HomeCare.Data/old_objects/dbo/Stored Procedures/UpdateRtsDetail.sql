...
Print 30.0    
   ...
   INSERT INTO ReferralTimeSlotDetails                        
     (ReferralTimeSlotMasterID,Day,StartTime,EndTime,UsedInScheduling,Notes,AnyTimeClockIn,CreatedBy,CreatedDate,UpdatedBy,UpdatedDate,SystemID,CareTypeId)                        
     SELECT @ReferralTimeSlotMasterID,T.Day,@StartTime,@EndTime,@UsedInScheduling,@Notes,@AnyTimeClockIn,@loggedInUserId,GETUTCDATE(),@loggedInUserId,GETUTCDATE(),@SystemID,@CareTypeId        
     FROM  @TempTable T         
...
Print 30.2    
  ...
  SELECT T.ReferralID,T.ReferralTimeSlotMasterID, T.ReferralTSDate,T.ReferralTSStartTime,T.ReferralTSEndTime,      
   ...
   ReferralTSEndTime=Case when @EndTime<@StartTime then DATEADD(Day,1,CONVERT(DATETIME, CONVERT(CHAR(8), T1.IndividualDate, 112) + ' ' + CONVERT(CHAR(8), EndTime, 108))) else      
   CONVERT(DATETIME, CONVERT(CHAR(8), T1.IndividualDate, 112) + ' ' + CONVERT(CHAR(8), EndTime, 108)) end ,        
