...
CREATE PROCEDURE [adc].[API_AddNoteByCareType_Temporary] 
...
BEGIN TRANSACTION trans                                                                                            
 BEGIN TRY              
  ...
  INSERT INTO @Temp    (RankOrder ,ReferralID ,AHCCCSID ,CISNumber ,ServiceDate ,ClockInTime ,ClockOutTime , ServiceCodeID ,ServiceCode , ServiceName ,Description ,      
  MaxUnit ,DailyUnitLimit ,UnitType ,PerUnitQuantity ,ServiceCodeType ,ServiceCodeStartDate ,          
 ServiceCodeEndDate ,IsBillable ,EmployeeID ,  EmployeeFirstName ,EmployeeLastName ,EmployeeUniqueID ,Address ,City ,StateCode ,ZipCode ,PayorID ,PayorName ,      
 PayorShortName ,PayorAddress ,PayorIdentificationNumber ,PayorCity ,PayorState ,PayorZipCode ,        
EmployeeVisitNoteID ,NoteID ,POSStartDate ,POSEndDate ,Rate ,SumOfServiceTime , ModifierID , AddNewNote ,RoundUpUnit ,ReferralBillingAuthorizationID,RenderingProvider_TaxonomyCode,SumOfUnit)       
...
WHERE EV.EmployeeVisitID =@EMPLOYEEVISITID  and ev.isDeleted=0 and ev.isPCACompleted=1 --AND (N.IsDeleted=0 OR N.IsDeleted IS NULL)                
  ...
  --EmployeeID,@BillingProvider_NM103_NameLastOrOrganizationName,EmployeeFirstName,@BillingProviderAddress,@BillingProviderCity,@BillingProviderState,@BillingProviderZipcode,              
