...
CAST((SELECT CONCAT(RIGHT('0' + RTRIM(MONTH(RM.CreatedDate)), 2), '/',RIGHT('0' + RTRIM(YEAR(RM.CreatedDate)), 4))) as varchar) AS AdmissionDate,
CONVERT(varchar, RM.CreatedDate, 8) AS Time, convert(varchar,RM.CreatedDate,101) as CreatedDate, RM.Unit, CONCAT(RM.Dose, ' ',RM.Unit) AS DoseWithUnit
