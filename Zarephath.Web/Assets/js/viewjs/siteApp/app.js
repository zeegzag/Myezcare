var app = angular.module("myApp", ['ngLoadingSpinner', 'angularUtils.directives.dirPagination', 'ngJsTree', 'cgBusy', 'ngSanitize', 'summernote', 'signature', 'dndLists', 'angular-steps']);
...
app.filter('time12hrFormat', function () {
    return function (dateString) {
        //var timeObj = moment.utc(dateString, "hh:mm A");
        return dateString != null ? moment.utc(dateString, "hh:mm A").format('hh:mm A') : '';
    };
});

app.filter('monthformat', function () {
    return function (dateString, format) {
        return dateString != null ? moment(dateString).format(format ? format : 'MMM-YYYY') : '';
    };
});

app.filter('yearformat', function () {
    return function (dateString, format) {
        return dateString != null ? moment(dateString).format(format ? format : 'YYYY') : '';
    };
});
...
app.filter('twodatetimeformathtml', ['$sce', function ($sce) {
    return function (dateString, dateString2, format) {
        var htmlCode;
        if (moment(dateString).format('MM/DD/YYYY') === moment(dateString2).format('MM/DD/YYYY')) {
            ...
            var d1Str = dateString != null ? moment(dateString).format('H:mm') : '';
            var d2Str = dateString2 != null ? moment(dateString2).format('H:mm') : '';
            htmlCode = d1Str + " - " + d2Str;

        } else {

            var d1Str = dateString != null ? moment(dateString).format(format ? format : 'H:mm') : '';
            var d2Str = dateString2 != null ? moment(dateString2).format(format ? format : 'H:mm') : '';
            htmlCode = d1Str + " - " + d2Str + ' <span class="sch-tdt-badge pull-right">+D</span>';
        }
