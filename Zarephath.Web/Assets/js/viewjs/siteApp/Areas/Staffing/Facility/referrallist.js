...
controllers.ReferralListController = function ($scope, $http, $window, $timeout) {
   ...
   // $scope.EncryptedReferralID = "m6-gKuJrVRNLZL08U5CmHg2";
    ...
    $scope.SaveAssignee = function (newAssignee, referral) {
        ...
        return AngularAjaxCall($http, HomeCareSiteUrl.UpdateAssigneeURL, jsonData, "Post", "json", "application/json").success(function (response) {
            ShowMessages(response);
            if (response.IsSuccess) {
                $scope.ReferralList = response.Data.Items;
                $scope.ReferralListPager.currentPageSize = response.Data.Items.length;
    ...
    $scope.ReferralEditModel = function (EncryptedReferralID, title) {
        ...
        $('#Referral_fixedAsidelDDLBindIFrame').attr('src', HomeCareSiteUrl.PartialAddfacilityURL + EncryptedReferralID);
    ...
    $scope.ShowReferralChartForm = function (referral, mode) {
        $scope.ReferralID = referral.ReferralID;
        $scope.OrganizationID = window.OrgID;
      var ReferralChartURL = HomeCareSiteUrl.OrbeonLoadHtmlFormURL + '?FormURL=' + '/ezcare/patient_chart' + '/' + 'pdf' + "?form-version=" + '1'
            + "&orbeon-embeddable=true"
            + "&OrgPageID=" + "EmployeeVisitTaskForm"
            + "&IsEditMode=" + "false"
            + "&ReferralID=" + $scope.ReferralID
            + "&FormId=" + "10000057"
            + "&FormName=" + "Patient%20Chart"
