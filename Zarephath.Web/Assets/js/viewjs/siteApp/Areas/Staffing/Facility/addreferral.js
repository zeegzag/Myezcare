...
controllers.AddReferralController = function ($scope, $http, $window, $timeout, $filter) {
    ...
    $("#liClient").removeClass("open");
    $("#liClient").removeClass("active");
    $("#liClient span.arrow").removeClass("open");

    $("#liFacility").addClass("open");
    $("#liFacility").addClass("active");
    $("#liFacility span.arrow").addClass("open");
    $("#liFacility ul.sub-menu").css('display', 'block');
    ...
    $scope.Save = function (isSaveAsDraft) {
        if (isSaveAsDraft) {
            ...
            var isValid = true;
            $("#txtFacilityName").removeClass("tooltip-danger");
            if ($scope.ReferralModel.Referral.FirstName == null || $scope.ReferralModel.Referral.FirstName == "") {
                $("#txtFacilityName").addClass("tooltip-danger");
                return false;
            }

            try {
                if ($scope.ReferralModel.Referral.FirstName == null || $scope.ReferralModel.Referral.AHCCCSID == null || $scope.ReferralModel.Referral.ReferralStatusID == null || $scope.ReferralModel.Referral.Assignee == null || $scope.ReferralModel.Referral.FirstName == "" || $scope.ReferralModel.Referral.AHCCCSID == "" || $scope.ReferralModel.Referral.ReferralStatusID == "" || $scope.ReferralModel.Referral.Assignee == "") {
    ...
    $scope.SaveReferral = function (isSaveAsDraft, notifyCm) {
        ...
        if (isHavePrimaryDxCode) {
            ...
            $scope.ReferralModel.Referral.BMI = null;//document.getElementById("bmi-results").value;
            $scope.ReferralModel.Referral.LastName = " ";
            $scope.ReferralModel.Referral.MiddleName = " ";
            $scope.ReferralModel.Referral.Gender = " ";
            $scope.ReferralModel.Referral.CISNumber = " ";
            $scope.ReferralModel.Referral.Dob = "01/01/1900";

            if (false) {//if ($scope.ReferralModel.Referral.Dob == "" || $scope.ReferralModel.Referral.Dob == null) {
                // "Akhil"
                ...
                AngularAjaxCall($http, $scope.AddReferralURL, jsonData, "Post", "json", "application/json").success(function (response) {
                    if (response.IsSuccess) {
                        // $scope.GroupName = $scope.ReferralModel.Referral.FirstName;
                        $scope.SaveEmpGroup($scope.ReferralModel.Referral.FirstName, response.Data.Referral.ReferralID, response.Data.Referral.IsEditMode);
                       
                       // $scope.SaveAndGetFacilityDetail(response.Data.Referral.EncryptedReferralID, 0);
                        response.Message = "Facility saved successfully";
                       ...
                       // SetMessageForPageLoad(response.Message, "ShowAddReferralMessage");
                        ...
                        response.Message = "Facility Address is required";
                    //ShowMessages(response);
                });
            }
        }
...
$scope.DeletePhysician = function (tempObject, title) {
            ...
            var jsonData = angular.toJson({
                ReferralMedication: {
                   ...
                   // HealthDiagnostics: $scope.ReferralMedication.HealthDiagnostics,
                    ...
                    AddMedicationModel: {
                        MedicationName: $scope.ReferralMedication.AddMedicationModel.MedicationName,
                        Generic_Name: $scope.ReferralMedication.AddMedicationModel.Generic_Name,
                        Brand_Name: $scope.ReferralMedication.AddMedicationModel.Brand_Name,
                        Product_Type: $scope.ReferralMedication.AddMedicationModel.Product_Type,
                        Route: $scope.ReferralMedication.AddMedicationModel.Route,
                        Dosage_Form: $scope.ReferralMedication.AddMedicationModel.Dosage_Form
                    }
                }
            });
    ...
    $scope.SaveAndGetFacilityDetail = function (referralID, value) {
        var objData = {
            ReferralId: referralID,
            TIN_Number: $scope.TIN_Number,
            EIN_Number: $scope.EIN_Number,
            Type: value
        };
        AngularAjaxCall($http, HomeCareSiteUrl.SaveAndGetFacilityDetails, objData, "Post", "json", "application/json").success(function (response) {
            if (response.IsSuccess) {
                $scope.TIN_Number = response.Data.TIN_Number;
                $scope.EIN_Number = response.Data.EIN_Number;
            }
        });
    }
    $scope.CheckReferralID = function () {
        if ($scope.ReferralModel.Referral.ReferralID > 0) {
            $scope.SaveAndGetFacilityDetail($scope.ReferralModel.Referral.EncryptedReferralID, 1);
        }
    }

    $scope.CheckReferralID();
    $scope.ContactListPage = [];
    $scope.Group = {};
    $scope.GetEmployeeGroup = function (GroupID) {
        $scope.GroupID = GroupID;
        var jsonData = angular.toJson({
            GroupID: GroupID
        });
        AngularAjaxCall($http, HomeCareSiteUrl.GetEmployeeGroupURL, jsonData, "Post", "json", "application/json", true).success(function (response) {
            ShowMessages(response);
            if (response.IsSuccess) {
                $scope.ContactListPage = response.Data;
                $scope.Step1 = false;
                $scope.Step2 = true;
   ...
   // $scope.GetEmployeeGroup("152")
    $scope.EmployeeDetails = function (data) {
      //  $scope.EMP_DDMasterID = data.Value;
      ////  $scope.EMP_DDMasterTypeID = data.DDMasterTypeID;
      //  $scope.SelectedEmployeeIds = [];
      //  var jsonData = angular.toJson({ groupIds: data.Value });
      //  AngularAjaxCall($http, HomeCareSiteUrl.GetEmployeeByGroupIdUrl, jsonData, "Post", "json", "application/json").success(function (response) {
      //      if (response.IsSuccess) {
      //          angular.forEach(response.Data, function (value, key) {
      //              $scope.SelectedEmployeeIds.push(value.EmployeeID);
      //              $("#cbEmployee" + value.EmployeeID).click();
      //          });
      //      }
      //  });
      //  $('#modal_AddEmployee').modal('show');
        $('#modal_AddEmployee').modal({
            backdrop: 'static',
            keyboard: false});
    }
    $scope.SelectedEmployeeIds = []
    ...
    $scope.AssignEmployee = function (IsChecked) {
        $scope.SelectedEmployeeIds = $scope.SelectedEmployeeIds.toString();
        var jsonData = angular.toJson({ GroupID: $scope.GroupID, EmployeeIDs: $scope.SelectedEmployeeIds, IsChecked: IsChecked });
        AngularAjaxCall($http, HomeCareSiteUrl.UpdateEmployeeGroupIdUrl, jsonData, "Post", "json", "application/json").success(function (response) {
            response.Message = "Employee allocated successfully";
            if (response.IsSuccess) {
                ShowMessages(response);
                $scope.ContactListPage.EmployeeListAssignedGroup = response.Data;
                $scope.SelectedEmployeeIds = [];
            }
        });
    }
    $scope.EmployeeGroup = {};
    $scope.SaveEmpGroup = function (GroupName, ReferralID,IsEditMode) {
        var jsonData = angular.toJson({ GroupName: GroupName, ReferralID: ReferralID, IsEditMode: IsEditMode });
        AngularAjaxCall($http, HomeCareSiteUrl.GroupNameUrl, jsonData, "Post", "json", "application/json").success(function (response) {
            if (response.IsSuccess) {
                $scope.EmployeeGroup = response.Data;
                $scope.GetEmployeeGroup(response.Data.Value);
    ...
    $scope.RemoveAllAssignedGroup = function (item) {
        var jsonData = angular.toJson({ GroupID: item.GroupIDs, EmployeeID:item.EmployeeID});
        AngularAjaxCall($http, HomeCareSiteUrl.RemoveAllAssignedGroupUrl, jsonData, "Post", "json", "application/json").success(function (response) {
            response.Message = "Employee allocated successfully";
            if (response.IsSuccess) {
                $scope.ContactListPage.EmployeeListAssignedGroup = response.Data;
                $scope.GetEmployeeGroup(item.GroupIDs);
            }
        });
    }
