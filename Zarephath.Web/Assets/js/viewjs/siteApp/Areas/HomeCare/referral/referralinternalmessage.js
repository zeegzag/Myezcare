...
controllers.ReferralInternalMessageController = function ($scope, $http, $window, $timeout) {
    ...
    $scope.SetReferralInternamMessageDetail = function (isSearchFilter) {
        ...
        AngularAjaxCall($http, HomeCareSiteUrl.SetReferralInternalMessageListURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
            $scope.IsEditNote = false;
    ...
    $scope.SaveReferralInternalMessage = function () {
        ...
        if (CheckErrors("#frmNewReferralInternalMessage")) {
            AngularAjaxCall($http, HomeCareSiteUrl.SaveReferralInternalMessageURL, jsonData, "Post", "json", "application/json").success(function (response) {
    ...
    $scope.ResolveNote = function (referralInternalMessage) {
        bootboxDialog(function (result) {
            if (result != null) {
                var jsonData = angular.toJson({ EncryptedReferralInternalMessageID: referralInternalMessage.EncryptedReferralInternalMessageID, ReferralID: referralInternalMessage.ReferralID, ResolvedComment: result });
                AngularAjaxCall($http, HomeCareSiteUrl.ResolveReferralInternalMessageURL, jsonData, "Post", "json", "application/json").success(function (response) {
                    ShowMessages(response);
                    if (response.IsSuccess) {
                        referralInternalMessage.IsResolved = response.Data.IsResolved;
    ...
    $scope.DeleteNote = function (referralInternalMessage) {
        bootboxDialog(function (result) {
            if (result) {
                ...
                AngularAjaxCall($http, HomeCareSiteUrl.DeleteReferralInternalMessageURL, jsonData, "Post", "json", "application/json").success(function (response) {
                    if (response.IsSuccess) {
                        $scope.ReferralInternalMessageList = response.Data.Items;
                        $scope.ReferralInternalMessageListPager.currentPageSize = response.Data.Items.length;
                        $scope.ReferralInternalMessageListPager.totalRecords = response.Data.TotalItems;
