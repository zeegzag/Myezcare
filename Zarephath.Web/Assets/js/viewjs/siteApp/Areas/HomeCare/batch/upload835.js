...
controllers.Upload835Controller = function ($scope, $http) {
    vm = $scope;
    $scope.Upload835FileListURL = HomeCareSiteUrl.Upload835FileListURL;
    $scope.Upload835List = [];
    ...
    $scope.GetUpload835List = function (isSearchDataMappingRequire) {
        ...
        AngularAjaxCall($http, HomeCareSiteUrl.Upload835FileListURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
    ...
    $scope.DeleteUpload835File = function (upload835FileId, title) {
        ...
        bootboxDialog(function (result) {
            ...
            if (result) {
                ...
                var jsonData = $scope.SetPostData($scope.Upload835ListPager.currentPage);
                AngularAjaxCall($http, HomeCareSiteUrl.DeleteUpload835FileUrl, jsonData, "Post", "json", "application/json").success(function (response) {
                    if (response.IsSuccess) {
                        $scope.Upload835List = response.Data.Items;
                        $scope.Upload835ListPager.currentPageSize = response.Data.Items.length;
                        $scope.Upload835ListPager.totalRecords = response.Data.TotalItems;
    ...
    $scope.ProcessUpload835File = function (upload835FileId, title) {
        ...
        bootboxDialog(function (result) {
            if (result) {
                ...
                var jsonData = $scope.SetPostData($scope.Upload835ListPager.currentPage);
                AngularAjaxCall($http, HomeCareSiteUrl.ProcessUpload835FileUrl, jsonData, "Post", "json", "application/json").success(function (response) {
                    if (response.IsSuccess) {
                        $scope.Upload835List = response.Data.Items;
                        $scope.Upload835ListPager.currentPageSize = response.Data.Items.length;
                        $scope.Upload835ListPager.totalRecords = response.Data.TotalItems;
    ...
    $scope.UploadFile = HomeCareSiteUrl.SaveUpload835FileUrl;
    ...
    $scope.BeforeSend = function (e, data) {
        ...
        $.each(data.files, function (index, file) {
            ...
            //    isValidFile = false;
            //}
            if (extension !== "edi" && extension !== "txt" && extension !== "csv") {
                ShowMessage(window.Invalid835UploadMessage.replace("{0}", extension), "error");
                errorMsg = window.Invalid835UploadMessage.replace("{0}", extension);
                isValidFile = false;
            }
    ...
    $scope.OnAdd = function (e, data) {
        ...
        $scope.Upload835Model.FileList.push(data.files[0]);

        if ($scope.Upload835Model.FileList.length == 1) {
            $scope.Upload835Model.SelectFileLabel = $scope.Upload835Model.FileList[0].name;
        } else {
            $scope.Upload835Model.SelectFileLabel = ($scope.Upload835Model.FileList.length) + " file(s) selected";
        }

        //$scope.Upload835Model.TempFilePath = data.files[0].name;

        if (counter !== $scope.Upload835Model.FileList.length && counter !== 0) {
            $scope.Upload835Model.TempFilePath += ","
        }

        counter += 1;

        if (!($scope.Upload835Model.TempFilePath)) {
            $scope.Upload835Model.TempFilePath = "";
        }
    ...
    $scope.Upload835File = function () {
        if (CheckErrors($($scope.FormID))) {
            $scope.FileData.files = $scope.Upload835Model.FileList;
            $scope.FileData.formData = {
                ...
                FileList: $scope.Upload835Model.FileList
            };
    ...
    $scope.SaveComment = function (comment, upload835) {
        ...
        return AngularAjaxCall($http, HomeCareSiteUrl.SaveUpload835CommentUrl, { upload835CommentModel: model }, "Post", "json", "application/json").success(function (response) {
            ShowMessages(response);
            if (response.IsSuccess) {
                $scope.Upload835ListPager.getDataCallback();
            }
