...
controllers.DxCodeListController = function ($scope, $http, $timeout) {
    ...
    $scope.AddDxCodeURL = HomeCareSiteUrl.AddDxCodeURL;
    $scope.DxCodeList = [];
    ...
    $scope.GetDxCodeList = function (isSearchDataMappingRequire) {
        ...
        AngularAjaxCall($http, HomeCareSiteUrl.GetDxCodeList, jsonData, "Post", "json", "application/json").success(function (response) {
    ...
    $scope.DeleteDxCode = function (dxCodeId, title) {
        ...
        bootboxDialog(function (result) {
            if (result) {
                ...
                var jsonData = $scope.SetPostData($scope.DxCodeListPager.currentPage);
                AngularAjaxCall($http, HomeCareSiteUrl.DeleteDxCode, jsonData, "Post", "json", "application/json").success(function (response) {
                    ShowMessages(response);
                    if (response.IsSuccess) {
                        $scope.DxCodeList = response.Data.Items;
                        $scope.DxCodeListPager.currentPageSize = response.Data.Items.length;
    ...
    $scope.DxCodeListPager.getDataCallback();
    $scope.DxCodeEditModel = function (EncryptedDXCodeID, title) {
        var EncryptedDXCodeID = EncryptedDXCodeID;
        $('#DxCode_fixedAside').modal({ backdrop: 'static', keyboard: false });
        $('#DxCode_fixedAsidelDDLBindIFrame').attr('src', HomeCareSiteUrl.PartialAddDxCodeURL + EncryptedDXCodeID);
    }
    $scope.DxCodeEditModelClosed = function () {
        $scope.Refresh();
        $('#DxCode_fixedAside').modal('hide');
    }
