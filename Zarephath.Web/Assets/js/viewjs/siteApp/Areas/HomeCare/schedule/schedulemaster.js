...
controllers.ScheduleMasterListController = function ($scope, $http, $timeout, $filter) {
    ...
    $scope.GetScheduleMasterList = function (isSearchDataMappingRequire) {
        ...
        AngularAjaxCall($http, HomeCareSiteUrl.GetScheduleMasterListURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
            if (response.IsSuccess) {
                $timeout(function () {
                    //
    ...
    $scope.SelectScheduleMaster = function (schedule) {
        schedule.IsChecked = schedule.IsChecked && !schedule.EmployeeVisitID;
    ...
    $scope.SelectAll = function () {
        ...
        angular.forEach($scope.ScheduleMasterList, function (item, key) {
            item.IsChecked = $scope.SelectAllCheckbox && !item.EmployeeVisitID;
            if (item.IsChecked) {
    ...
    $scope.DeleteSchedule = function (data) {
        bootboxDialog(function (result) {
            if (result) {
                ...
                var jsonData = $scope.SetPostData($scope.ScheduleMasterListPager.currentPage);
                AngularAjaxCall($http, HomeCareSiteUrl.DeleteScheduleMasterURL, jsonData, "Post", "json", "application/json").success(function (response) {
