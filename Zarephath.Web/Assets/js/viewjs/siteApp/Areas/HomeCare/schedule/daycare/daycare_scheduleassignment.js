...
controllers.ScheduleAssignmentController = function ($scope, $http, $compile, $timeout, $filter) {
    ...
    $scope.CreateCalanderEventModel = function (data) {
        var color = "#6aa84f !important";  //        $scope.colors[data.ScheduleStatusID - 1];
        //var editable = true;
        var editable = false;
        var isPastEvent = false;
        var isTodayEvent = false;
        ...
        if (moment(data.StartDate).format('YYYYMMDD') == moment(new Date()).format('YYYYMMDD')) {
            isTodayEvent = true;
        }
    ...
    $scope.RefetchResources = function (calendarObj,callback) {
        ...
        AngularAjaxCall($http, HomeCareSiteUrl.DayCare_GetReferralForSchedulingURL, data, "Post", "json", "application/json", false).success(function (response) {
            if (response.IsSuccess) {
    ...
    $scope.GetScheduleList = function (calendarObj, start, end, callback) {
        ...
        if ($scope.SelectedReferralList.length > 0) {
            ...
            AngularAjaxCall($http, HomeCareSiteUrl.DayCare_GetScheduleListByReferralsURL, jsonData, "Post", "json", "application/json", $scope.ShowLoadOnCalenderLoad).success(function (response) {
    ...
    $scope.EventRender = function (calendarObj, event, ele) {
        ...
        if (scheduleModel != null) {
            ...
            newScope.DataModel = {
                ...
                TogglePatientAttendedSchedule: $scope.TogglePatientAttendedSchedule
            };
    ...
    $scope.TogglePatientAttendedSchedule = function (calendarObj, event, op) {
        var attended = (op == 'a' ? false : true);

        if ((event.isPastEvent || event.isTodayEvent) && attended !== event.scheduleModel.TempIsPatientAttendedSchedule) {
            event.scheduleModel.TempIsPatientAttendedSchedule = attended;
            $scope.PatientAttendanceActionModal(calendarObj, event);
        }
    }
