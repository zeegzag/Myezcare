...
controllers.Reconcile835Controller = function ($scope, $http, $timeout) {
    ...
    $scope.Reconcile835ListURL = SiteUrl.Reconcile835ListURL;
    ...
    $scope.GetReconcile835List = function (isSearchDataMappingRequire) {
        ...
        AngularAjaxCall($http, SiteUrl.Reconcile835ListURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
    ...
    $scope.GetBatchNoteDetails = function (item, runAlways) {
        if (item.CSModel == undefined || runAlways) {
            ...
            AngularAjaxCall($http, SiteUrl.GetReconcileBatchNoteDetailsUrl, jsonData, "Post", "json", "application/json").success(function (response) {
                if (response.IsSuccess) {
    ...
    $scope.MarkNoteAsLatest = function (data, item) {
        ...
        AngularAjaxCall($http, SiteUrl.MarkNoteAsLatestUrl, jsonData, "Post", "json", "application/json").success(function (response) {
            if (response.IsSuccess) {
    ...
    //            AngularAjaxCall($http, SiteUrl.SetClaimAdjustmentFlagUrl, jsonData, "Post", "json", "application/json").success(function (response) {
    //                
    //                if (response.IsSuccess) {
    ...
    $scope.GetUpload835URL = SiteUrl.GetUpload835FilesURL;
    ...
    $scope.ExportReconcileList = function () {
        ...
        var jsonData = $scope.SetPostData($scope.Reconcile835ListPager.currentPage);
        AngularAjaxCall($http, SiteUrl.GetExportReconcile835ListListURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
            if (response.IsSuccess) {
    ...
    $scope.BulkSubmitClaimAdjustmentReason = function () {
        ...
        AngularAjaxCall($http, SiteUrl.BulkSetClaimAdjustmentFlagUrl, jsonData, "Post", "json", "application/json").success(function (response) {
            if (response.IsSuccess) {
                if ($scope.SubmitClaimAdjustmentReasonModel.AdjustmentType === window.AdustmentType_Remove) {
                    $scope.SubmitClaimAdjustmentReasonModel.AdjustmentType = null;
                    $scope.SubmitClaimAdjustmentReasonModel.AdjustmentReason = null;
