...
controllers.ReferralDocumentController = function ($scope, $http, $window, $timeout) {
    ...
    $scope.SetReferralMissingDocument = function () {
        ...
        AngularAjaxCall($http, SiteUrl.SetReferralMissingDocumentURL, jsonData, "Post", "json", "application/json").success(function (response) {
    ...
    $scope.SendEmail = function () {
        if (CheckErrors("#frmMissingDocument")) {
            ...
            AngularAjaxCall($http, SiteUrl.SendEmailForReferralMissingDocumentURL, jsonData, "Post", "json", "application/json").success(function (response) {
                ShowMessages(response);
            });
        }
    };
    ...
    $scope.UploadFile = SiteUrl.UploadFile;
    ...
    $scope.AfterSend = function (data) {
        ...
        AngularAjaxCall($http, SiteUrl.SaveReferralDocumentURL, jsonData, "Post", "json", "application/json", false).success(function (response) {
            if (response.IsSuccess) {
    ...
    $scope.GetReferralDocumentList = function () {
        ...
        AngularAjaxCall($http, SiteUrl.GetReferralDocumentList, jsonData, "Post", "json", "application/json", false).success(function (response) {
            if (response.IsSuccess) {
                $scope.ReferralDocumentList = response.Data.Items;
    ...
    $scope.DeleteDocument = function (data) {
        bootboxDialog(function (result) {
            if (result) {
                ...
                AngularAjaxCall($http, SiteUrl.DeleteDocumentURL, jsonData, "Post", "json", "application/json").success(function (response) {
                    if (response.IsSuccess) {
                        $scope.ReferralDocumentList = response.Data.Items;
                        $scope.ReferralDocumentPager.currentPageSize = response.Data.Items.length;
                        $scope.ReferralDocumentPager.totalRecords = response.Data.TotalItems;
    ...
    $scope.SaveDocument = function (data) {
        ...
        if (CheckErrors("#frmDocumentEdit")) {
            AngularAjaxCall($http, SiteUrl.EditDocumentURL, jsonData, "Post", "json", "application/json").success(function (response) {
                if (response.IsSuccess) {
                    $scope.ReferralDocumentList = response.Data.Items;
                    $scope.ReferralDocumentPager.currentPageSize = response.Data.Items.length;
                    $scope.ReferralDocumentPager.totalRecords = response.Data.TotalItems;
