...
controllers.AddReferralController = function ($scope, $http, $window, $timeout, $filter) {
    ...
    $scope.SaveReferralDXCodeMapping = function () {
        if ($scope.ReferralModel.ReferralDXCodeMapping.DXCodeID && $scope.ReferralModel.ReferralDXCodeMapping.StartDate && $scope.ReferralModel.ReferralDXCodeMapping.Precedence) {
            var precedenceCount = $scope.ReferralModel.DxCodeMappingList.filter(function (item) {

                var sd = $scope.ReferralModel.ReferralDXCodeMapping.StartDate;
                var ed = $scope.ReferralModel.ReferralDXCodeMapping.EndDate;
                var prcd = parseInt($scope.ReferralModel.ReferralDXCodeMapping.Precedence);
                var dxId = $scope.ReferralModel.ReferralDXCodeMapping.DXCodeID;

                var isd = item.StartDate;
                var ied = item.EndDate;
                var iprcd = parseInt(item.Precedence);
                var idxId = item.DXCodeID;


                if (
                    (ValideElement(ed) === false && sd >= isd) || (ValideElement(ied) === false && sd >= isd)
                    || (sd >= isd && sd <= ied) || (ed >= isd && ed <= ied) || (sd < isd && ed > ied)
                ) {
                    return iprcd === prcd;// && idxId != dxId;
                }


               
                //if (
                //    ($scope.ReferralModel.ReferralDXCodeMapping.StartDate >= item.StartDate && $scope.ReferralModel.ReferralDXCodeMapping.StartDate <= item.EndDate)
                //        ||
                //    ($scope.ReferralModel.ReferralDXCodeMapping.EndDate >= item.StartDate && $scope.ReferralModel.ReferralDXCodeMapping.EndDate <= item.EndDate)
                //        ||
                //    ($scope.ReferralModel.ReferralDXCodeMapping.StartDate < item.StartDate && $scope.ReferralModel.ReferralDXCodeMapping.EndDate > item.EndDate)
                //) {
            ...
            if (precedenceCount > 0) {
                ...
                var index = $.map($scope.ReferralModel.DxCodeMappingList, function (obj, id) {
                    if (obj.DXCodeID == $scope.ReferralModel.ReferralDXCodeMapping.DXCodeID) {
                        return id;
                    }
                    return null;
                });
                if (index.length > 0 && $scope.ReferralModel.DxCodeMappingList[index].DXCodeID == $scope.ReferralModel.ReferralDXCodeMapping.DXCodeID) {
