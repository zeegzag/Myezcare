var encvm;
controllers.RptEncounterPrintController = function ($scope, $http, $window) {
    encvm = $scope;
    $scope.NoRecordsFoundFlag = false;
    $scope.newInstance = function () {
        return $.parseJSON($("#hdnSetEncounterPrintModel").val());
    };

    $scope.GetReferralInfoforReportURL = SiteUrl.GetReferralInfoforReportURL;

    $scope.SearchEncounterPrintModel = $.parseJSON($("#hdnSetEncounterPrintModel").val()).SearchEncounterPrintModel;
    $scope.EncounterTokenObj = {};
    $scope.DownloadEncounterReportReport = function (ele) {
        $scope.EPAjaxCall = true;
        $scope.NoRecordsFoundFlag = false;
        $(ele).button('loading');
        var jsonData = angular.toJson({ searchEncounterPrintModel: $scope.SearchEncounterPrintModel });
        AngularAjaxCall($http, SiteUrl.GetEncounterPrintReportUrl, jsonData, "Post", "json", "application/json", false).success(function (response) {
    ...
    $scope.ResetSearchFilter = function () {
        $scope.SearchEncounterPrintModel = {};;
    ...
    $scope.RemoveReferral = function () {
        $scope.SearchEncounterPrintModel.ReferralID = null;
...
controllers.RptEncounterPrintController.$inject = ['$scope', '$http'];
