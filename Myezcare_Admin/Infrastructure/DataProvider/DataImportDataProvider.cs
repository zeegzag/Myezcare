...
namespace Myezcare_Admin.Infrastructure.DataProvider
{
    ...
    {
        public DataImportDataProvider(MyEzcareOrganization org) : base(org)
        {
        ...
        public ServiceResponse ReadAndValidateMultipleSheets<T>(string path, string[] tableName, string[] columns, string spName, long userID, List<SearchValueData> customSPPara = null) where T : class, new()
        {
            ...
            try
            {
                using (SpreadsheetDocument spreadSheetDocument = SpreadsheetDocument.Open(path, false))
                {
                    ...
                    for (int index = 0; index < sheets.Count(); index++)
                    {
                        ...
                        sheetNames.Add(sheets.ElementAt(index).Name.Value);
                        string relationshipId = sheets.ElementAt(index).Id.Value;
                        WorksheetPart worksheetPart = (WorksheetPart)spreadSheetDocument.WorkbookPart.GetPartById(relationshipId);
                        Worksheet workSheet = worksheetPart.Worksheet;
                        SheetData sheetData = workSheet.GetFirstChild<SheetData>();
                        IEnumerable<Row> rows = sheetData.Descendants<Row>();
                ...
                string message = "";
                for (int tables = 0; tables < dataTables.Count; tables++)
                {
                    dataTables[tables].Rows.RemoveAt(0);
                    if (dataTables[tables].Rows.Count == 0)
                    {
                        message += string.Format("Sheet {0} does not contain any record(s).", sheetNames[tables]);
                        //response.IsSuccess = false;
                        //response.Message = "File does not contain any record(s).";
                        //return response;
                    }
                    ...
                    {
                        dataTables[tables] = RemoveEmptyRows(dataTables[tables]);
                        DeleteAllTempData(tableName[tables], userID);
                        string n = InsertData(tableName[tables], dataTables[tables], columns[tables], userID, true);
                    }
                }

                T objImport = GetMultipleEntity<T>(spName, customSPPara);
                response.Data = objImport;
            }
            catch (Exception ex)
            {
        ...
        public string InsertData(string tableName, DataTable table, string columns, long userID, bool addCreatedDate = false)
        {
            List<string> tColumns = columns.Split(',').ToList();
            string query = "Insert into " + tableName + " (CreatedBy," + (addCreatedDate ? "CreatedDate," : "") + columns + ") values ";
            ...
            foreach (DataRow dr in table.Rows)
            {
                ...
                for (int i = 0; i < tColumns.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        newRowQuery += "'" + userID + "',";
                        if(addCreatedDate)
                            newRowQuery += "'" + DateTime.Now.ToString(Constants.DbDateTimeFormat) + "',";
                    }
