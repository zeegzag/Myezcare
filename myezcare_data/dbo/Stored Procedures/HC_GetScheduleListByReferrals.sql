...
SELECT R.ReferralID, R.FirstName,R.LastName,C.Phone1,C.Phone2, C.Email,SM.ScheduleID,SS.ScheduleStatusName,RTD.ReferralTSDateID,              
 ...
 ,EV.ClockInTime, EV.ClockOutTime,EV.IVRClockIn,EV.IVRClockOut,EV.IsPCACompleted,
 (SELECT ShortName FROM Payors WHERE PayorID = SM.PayorID) AS Payor,
 (SELECT Title FROM DDMaster WHERE DDMasterID = SM.CareTypeId) AS CareType  , EV.IsApprovalRequired             
 FROM Referrals R              
 INNER JOIN ReferralTimeSlotDates RTD ON RTD.ReferralID=R.ReferralID AND (RTD.ReferralTSDate BETWEEN @StartDate AND @EndDate)              
 LEFT JOIN ScheduleMasters SM ON SM.IsDeleted = 0 AND  
   
 ( (SM.ReferralTSDateID=RTD.ReferralTSDateID) OR    
 (RTD.ReferralTSDate BETWEEN CONVERT(DATE,SM.StartDate) AND CONVERT(DATE,SM.EndDate) AND  SM.ReferralTSDateID IS NULL AND SM.EmployeeTSDateID IS NULL )  )   
