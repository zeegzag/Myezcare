-- EXEC RefreshBatchNotes01        
CREATE PROCEDURE [dbo].[RefreshBatchNotes01]        
AS        
BEGIN        
  ...
  DECLARE @TepmTable TABLE (BatchNoteID BIGINT,TempCalculatedUnit INT,TempCalculatedAmount DECIMAL(10,2));        
  INSERT INTO @TepmTable         
   ...
   FROM BatchNotes BN         
   INNER JOIN Batches B ON BN.BatchID = B.BatchID AND B.IsSent = 0        
   INNER JOIN @TepmTable AS A ON A.BatchNoteID = BN.BatchNoteID AND (ISFirstTimeClaimInBatch IS NULL OR ISFirstTimeClaimInBatch =1)          
   AND IsNewProcess = 1 AND IsUseInBilling = 1        
