...
BEGIN    
	...
	SELECT *,COUNT(t1.ReferralID) OVER() AS Count FROM 
		...
		ReferralID, ReferralName,AHCCCSID,CISNumber,Payor,Population,Title ,StartDate--,NoteID
		 FROM ( 
		 --SELECT DISTINCT R.ReferralID,F.FacilityID,F.FacilityName,  R.LastName+', '+R.FirstName as ReferralName,R.AHCCCSID,R.CISNumber,Payor=P.ShortName,R.Population,R.Title , 
		 SELECT DISTINCT R.ReferralID,  R.LastName+', '+R.FirstName as ReferralName,R.AHCCCSID,R.CISNumber,Payor=P.ShortName,R.Population,R.Title , 
		 ...
		 WHERE ((CAST(@IsDeleted AS BIGINT)=-1) OR N.IsDeleted=@IsDeleted) AND ((CAST(@IsBillable AS BIGINT)=-1) OR N.IsBillable=@IsBillable) 
		  ...
		  GROUP BY R.ReferralID,  R.LastName,R.FirstName ,R.AHCCCSID,R.CISNumber,P.ShortName,R.Population,R.Title 
