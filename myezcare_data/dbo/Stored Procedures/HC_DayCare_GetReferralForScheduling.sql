-- EXEC HC_GetReferralForScheduling @ReferralName = '',@StartDate = '2018/07/09', @EndDate = '2018/07/15'                
CREATE PROCEDURE [dbo].[HC_DayCare_GetReferralForScheduling]    
...
LEFT JOIN ReferralTimeSlotDates RTD ON RTD.ReferralID=T.ReferralID AND RTD.ReferralTSDate BETWEEN @StartDate AND @EndDate AND RTD.UsedInScheduling=0                      
 ...
 where     
 1=1    
 --AND ((@SchStatus= 1 and NewAllocatedHrs = NewUsedHrs) or (@SchStatus = 2 and NewAllocatedHrs != NewUsedHrs) or (@SchStatus = -1 or @SchStatus is null))  and NewAllocatedHrs is not null        
 ORDER BY LASTNAME ASC      
END
