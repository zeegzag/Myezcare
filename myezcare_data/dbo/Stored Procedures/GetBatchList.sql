-- EXEC GetBatchList @IsSentStatus = '-1', @SortExpression = 'BatchID', @SortType = 'DESC', @FromIndex = '1', @PageSize = '50'
 ...
 (                                                              
  ...
  (                                                              
   SELECT ROW_NUMBER() OVER (ORDER BY                                                              
	...
	(
	SELECT BH.BatchID,COUNT(DISTINCT BCH.NoteID) as Gathered, BH.StartDate,BH.EndDate,BHT.BatchTypeName,                                                            
	P.PayorName,EMP.UserName as GatheredBy,BH.CreatedDate as GatherDate,BH.SentDate  ,                                 
	--MIN(N.ServiceDate)  as ServiceStartDate,MAX(N.ServiceDate) as ServiceEndDate,
	...
	--(select count(DISTINCT NoteID) from BatchNotes where BatchID=BH.BatchID AND CLP02_ClaimStatusCode NOT IN (4,22))/NULLIF(count(BCH.BatchID),1)                     
	  
	  ,SUM(CONVERT(DECIMAL(10,2),ISNULL(N.CalculatedAmount,BND.CalculatedAmount))) as Amount1
	  ,SUM( CASE WHEN ISNULL(N.ServiceDate,BND.ServiceDate) >'2017-10-01' AND PSM.BillingUnitLimit IS NOT NULL AND ISNULL(N.CalculatedUnit,BND.CalculatedUnit) > PSM.BillingUnitLimit 
	        THEN CONVERT(DECIMAL(10,2), ( ISNULL(N.CalculatedAmount,BND.CalculatedAmount) / ISNULL(N.CalculatedUnit,BND.CalculatedUnit)) * PSM.BillingUnitLimit ) ELSE CONVERT(DECIMAL(10,2), ISNULL(N.CalculatedAmount,BND.CalculatedAmount)) END ) as Amount

	  FROM                                                                      
	  Batches BH        
	  INNER JOIN BatchTypes BHT on BH.BatchTypeID= BHT.BatchTypeID                           
	  LEFT JOIN BatchNotes BCH on BCH.BatchID=BH.BatchID                          
	  LEFT JOIN Notes N on N.NoteID=BCH.NoteID AND BH.IsSent=0
	  LEFT JOIN BatchNoteDetails BND ON BND.NoteID=BCH.NoteID AND BND.BatchID=BCH.BatchID   AND BH.IsSent=1
	  --LEFT JOIN Notes N on N.NoteID=BCH.NoteID AND BH.IsSent=0                                                                           
	  ...
	  LEFT JOIN PayorServiceCodeMapping PSM ON 
	  (  (BH.IsSent=0 AND PSM.PayorServiceCodeMappingID=N.PayorServiceCodeMappingID) OR (BH.IsSent=1 AND PSM.PayorServiceCodeMappingID=BND.PayorServiceCodeMappingID) )                         
	  --INNER JOIN PlaceOfServices PSM on PSM.PosID=N.PosID
	 ...
	 -- AND ((@BillingProviderIDS is null OR BH.BatchID in(select BatchID from BatchApprovedFacility where BillingProviderID in(select val from GetCSVTable(@BillingProviderIDS)))))                          
		...
		GROUP BY BH.BatchID,BCH.BatchID,BH.StartDate,BH.EndDate,BHT.BatchTypeName,P.PayorName,BH.CreatedDate ,BH.SentDate,EMP.UserName,BH.IsSent,EMPI.UserName                                                
	) AS T2                   
 ) AS T1                                                                                      
 )                                                                                        
