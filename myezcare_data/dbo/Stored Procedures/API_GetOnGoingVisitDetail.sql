...
BEGIN                              
 ...
 IF(@EmployeeVisitID IS NOT NULL)          
  SELECT ClockInTime,ClockOutTime,SurveyCompleted,IsSigned,IsPCACompleted,EmployeeVisitID,IVRClockOut,    
  --CASE WHEN UsedInScheduling=1 THEN 0 ELSE 1 END AS IsDenied,      
  IsDenied,Notes AS DeniedReason,CASE WHEN @Count>0 THEN 1 ELSE 0 END AS HasConclusion    
 ...
 ELSE          
  ...
  IsDenied,Notes AS DeniedReason          
