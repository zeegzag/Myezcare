...
---------OldCode End
 ...
 CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'TotalETSDetailCount' THEN TBL1.TotalETSDetailCount END END ASC,                      
   ...
   SELECT DISTINCT ets.EmployeeTimeSlotMasterID,ets.EmployeeID,Name=dbo.GetGeneralNameFormat(e.FirstName,e.LastName),
   CreatedBy=dbo.GetGeneralNameFormat(ec.FirstName,ec.LastName),UpdatedBy=dbo.GetGeneralNameFormat(eu.FirstName,eu.LastName),ets.CreatedDate,ets.UpdatedDate,ets.IsDeleted,
