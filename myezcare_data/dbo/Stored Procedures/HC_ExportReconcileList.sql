--EXEC HC_ExportReconcileList @ClaimStatusCodeID = '0', @Get835ProcessedOnly = '0', @Upload835FileID = '0', @ServiceID = '-1', @ClaimAdjustmentTypeID = '-1', @NoteID = '', @PayorClaimNumber = '', @SortExpression = 'ServiceDate', @SortType = 'DESC', @FromIndex = '0', @PageSize = '0'
...
CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'ClientDob' THEN CONVERT(datetime, ClientDob, 103) END END ASC,                
  ...
  CASE WHEN @SortType = 'ASC' THEN CASE WHEN @SortExpression = 'LoadDate' THEN CONVERT(datetime, LoadDate, 103) END END ASC,                
   ...
   RowNumber FROM ( SELECT DISTINCT BN.CLP02_ClaimStatusCode,BN.S277,BN.S277CA,        
    ClientName= R.LastName+', '+R.FirstName,ClientDob= R.Dob,R.Gender, R.AHCCCSID, R.CISNumber, R.Population, R.Title,Payor=BND.PayorShortName,            
 ...
 INNER JOIN ServiceCodes SC ON SC.ServiceCodeID=N.ServiceCodeID  
    ...
    INNER JOIN Employees EC on EC.EmployeeID=BND.CreatedBy        
