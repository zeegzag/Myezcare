...
BEGIN                        
 ...
 LEFT JOIN ScheduleMasters sm ON sm.EmployeeTSDateID=etsd.EmployeeTSDateID AND sm.IsDeleted=0          
    ORDER BY Day ASC        
...
AND (CONVERT(DATE,sm.StartDate) BETWEEN @StartDate AND @EndDate OR CONVERT(DATE,sm.EndDate) BETWEEN @StartDate AND @EndDate) AND sm.IsDeleted=0    
