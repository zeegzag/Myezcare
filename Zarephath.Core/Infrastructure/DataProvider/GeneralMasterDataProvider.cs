...
namespace Zarephath.Core.Infrastructure.DataProvider
{
    public class GeneralMasterDataProvider : BaseDataProvider, IGeneralMasterDataProvider
    {
        ...
        public ServiceResponse SaveDDmaster(DDMaster ddMaster, List<long> childTaskIds, long loggedInUserId)
        {
            ...
            try
            {
                ...
                //searchList.Add(new SearchValueData("Value2", ddMaster.Value2));
                searchList.Add(new SearchValueData("ChildTaskIds", childTaskIds==null?"": Convert.ToString(string.Join(Constants.Comma, childTaskIds))));
                searchList.Add(new SearchValueData("IsMultiSelect", Convert.ToString(ddMaster.IsMultiSelect)));
            ...
            string sortIndex, string sortDirection, long loggedInUserId)
        {
            ...
            {
                response.IsSuccess = true;
                response.Message = string.Format(Resource.RecordDeletedSuccessfully, Resource.GeneralMaster);
            }
        ...
        public ServiceResponse CheckForParentChildMapping(DDMasterTypeModel model)
        {
            var response = new ServiceResponse();

            if(model.DDMasterTypeID<=0)
            {
                response.IsSuccess = false;
            ...
            List<SearchValueData> searchList =new List<SearchValueData>();
            searchList.Add(new SearchValueData { Name = "DDMasterTypeID", Value = Convert.ToString(model.DDMasterTypeID) });
            searchList.Add(new SearchValueData { Name = "DDMasterID", Value = Convert.ToString(model.DDMasterID) });
            ParentChildMappingModel data = GetMultipleEntity<ParentChildMappingModel>(StoredProcedure.CheckForParentChildMapping, searchList);
            response.IsSuccess = true;
            response.Data = data;
            return response;
        }
        ...
        //public ServiceResponse GetMasterDetailsApi(int ddTypeId)
        //{
        //    var response = new ServiceResponse();
        ...
        //    var ParentRecord = DDMasterTypeList.Where(x => x.DDMasterTypeID == ddTypeId).ToList();
