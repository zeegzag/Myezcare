...
namespace PetaPoco
{
    ...
    public partial class Database : IDisposable
    {
        ...
        public Database(MyEzcareOrganization organization)
        {
            ...
            {
                connectionstring = string.Format("Server={0};Database={1}; Trusted_Connection={2};{3}", organization.DBServer, organization.DBName, Trusted_Connection, enableEncryptConnection);
            }
            else
            {
                connectionstring = string.Format("Server={0};Database={1}; User ID={2};Password={3}", organization.DBServer, organization.DBName, organization.DBUserName, organization.DBPassword);
        ...
        public void BulkInsertRecords<T>(IEnumerable<T> collection)
        {
            try
            {
                ...
                using (var cmd = CreateCommand(_sharedConnection, ""))
                {
                    ...
                    string cols = string.Join(", ", (from c in pd.QueryColumns select tableName + "." + _dbType.EscapeSqlIdentifier(c)).ToArray());
    ...
    */
    ...
    /// </summary>
    [AttributeUsage(AttributeTargets.Class)]
    public class VirtualKeyAttribute : Attribute
    {
        public VirtualKeyAttribute(string virtualKey)
        {
            Value = virtualKey;
        }
