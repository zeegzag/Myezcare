    ...
    public class TypedSegmentDTP : TypedSegment
    {
        ...
        public DateTimePeriod DTP03_Date
        {
            ...
            set {
                ...
                string newformat = value.IsWithTime? "yyyyMMddHHmm" : "yyyyMMdd";

                string dateRange = "{0:" + newformat + "}-{1:" + newformat + "}";
                string dateOnly = "{0:" + newformat + "}";

                _segment.SetElement(3,
                                    value.IsDateRange
                                        ? String.Format(dateRange, value.StartDate, value.EndDate)
                                        : String.Format(dateOnly, value.StartDate));
            }
        }
